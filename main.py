"""
Visual Cortex — PPTX Generator API
Génère des présentations PowerPoint chartées à partir d'un template + prompt.

Modèle freemium :
  - Gratuit  : FREE_QUOTA_PER_IP générations par IP par jour
  - Pro      : illimité (token Bearer valide)

La clé API Claude est stockée côté serveur (variable d'env ANTHROPIC_API_KEY).
Elle n'est JAMAIS exposée au front-end.
"""
...
app = FastAPI(title="Visual Cortex API", version="2.0.0")
...
FREE_QUOTA_PER_IP = int(os.environ.get("FREE_QUOTA_PER_IP", "3"))  # générations gratuites/jour/IP
...
def build_pptx_from_template(pptx_bytes: bytes, content: dict) -> bytes:
    ...
    # Supprime les slides existantes
...
async def analyze_template(file: UploadFile = File(...)):
    """Analyse un template PPTX. Gratuit, sans quota consommé."""
...
async def generate_preview(
    ...
    """Génère le plan (titres des slides) sans créer le fichier. Consomme 1 quota."""
...
async def generate_presentation(
    ...
    Aucune clé API n'est requise côté client.
    Quota freemium géré côté serveur.
    """
    if not template.filename.endswith(".pptx"):
        raise HTTPException(400, "Le template doit être un .pptx")
    if nb_slides < 3 or nb_slides > 30:
