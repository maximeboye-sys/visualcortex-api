...
def layout_quote_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: plein fond accent1 — citation blanche + grand guillemet décoratif
        _h2_rect(slide, 0, 0, W, H, accent1)
        _h2_text(slide, '\u201c',
                 left=_LY.CL - 0.10, top=_LY.CT - 0.20,
                 width=2.60, height=2.10,
                 font=font, size_pt=180, color='FFFFFF', bold=True, align='left')
        _set_shape_alpha(slide.shapes[-1], 15)
        q_y3 = max(_LY.CT + 0.55, (H - quote_h) / 2 - 0.30)
        _h2_text(slide, quote,
                 left=_LY.CL + 0.60, top=q_y3,
                 width=_LY.CW - 1.20, height=quote_h + 0.40,
                 font=font, size_pt=22, color='FFFFFF',
                 bold=False, italic=True, align='center', line_spacing=1.35)
        sep_y3 = q_y3 + quote_h + 0.50
        _h2_rect(slide, left=W / 2 - 0.40, top=sep_y3, width=0.80, height=0.05, color='FFFFFF')
        _set_shape_alpha(slide.shapes[-1], 50)
        if author:
            _h2_text(slide, author,
                     left=_LY.CL, top=sep_y3 + 0.14,
                     width=_LY.CW, height=0.38,
                     font=font, size_pt=13, color='FFFFFF', bold=True, align='center')
        if source:
            _h2_text(slide, source,
                     left=_LY.CL, top=H - 0.75,
                     width=_LY.CW, height=0.28,
                     font=font, size_pt=9, color='FFFFFF', bold=False, align='center')
            _set_shape_alpha(slide.shapes[-1], 70)

    elif v == 4:
        # v4: split horizontal — bande accent haut + zone blanche bas
        split_y = H * 0.38
        _h2_rect(slide, 0, 0, W, split_y, accent1)
        _h2_text(slide, '\u201c',
                 left=_LY.CL, top=_LY.CT - 0.15,
                 width=1.80, height=1.50,
                 font=font, size_pt=110, color='FFFFFF', bold=True, align='left')
        _set_shape_alpha(slide.shapes[-1], 22)
        category4 = content.get('category', content.get('title', ''))
        if category4:
            _h2_text(slide, category4.upper(),
                     left=_LY.CL + 1.60, top=_LY.CT + 0.12,
                     width=_LY.CW - 1.60, height=0.26,
                     font=font, size_pt=9, color='FFFFFF', bold=True, align='left')
        q_y4 = split_y + 0.30
        _h2_text(slide, f'\u00ab {quote} \u00bb',
                 left=_LY.CL + 0.40, top=q_y4,
                 width=_LY.CW - 0.80, height=quote_h + 0.40,
                 font=font, size_pt=20, color=accent1,
                 bold=False, italic=True, align='left', line_spacing=1.30)
        if author:
            parts4 = author.split(',', 1)
            ay4 = q_y4 + quote_h + 0.55
            _h2_rect(slide, left=_LY.CL + 0.40, top=ay4, width=0.90, height=0.045, color=accent2)
            _h2_text(slide, parts4[0].strip(),
                     left=_LY.CL + 0.40, top=ay4 + 0.10,
                     width=_LY.CW - 0.80, height=0.36,
                     font=font, size_pt=13, color=dk1, bold=True, align='left')
            if len(parts4) > 1:
                _h2_text(slide, parts4[1].strip(),
                         left=_LY.CL + 0.40, top=ay4 + 0.48,
                         width=_LY.CW - 0.80, height=0.26,
...
def layout_col3_v4(prs: Presentation, content: dict, tp: dict):
    """
    3 colonnes enrichies.
    v0: fond carte + barre accent top + icon/label + titre + bullets + stat
    v1: en-tête pleine couleur (36% hauteur) + corps clair en bas
    v2: pilier numéroté — bordure gauche + grand numéro décoratif + titre + bullets
    ...
    for i, col in enumerate(columns[:n]):
        ...
        if v == 0:
            ...
            y = _LY.CT + bar_h + 0.14
            if icon:
                _h2_text(slide, icon, left=cx + _LY.PAD, top=y,
                         width=0.44, height=0.38,
                         font=font, size_pt=18, color=dk1, bold=False, align='left')
            if label:
                _h2_text(slide, label, left=cx + _LY.PAD + 0.46, top=y + 0.08,
                         width=card_w - _LY.PAD - 0.52, height=0.24,
                         font=font, size_pt=8, color='999999', bold=True, align='right')
            if icon or label:
                y += 0.44
            _h2_text(slide, title, left=cx + _LY.PAD, top=y,
                     width=card_w - _LY.PAD * 2, height=0.42,
                     font=font, size_pt=_LY.T_TITLE, color=dk1, bold=True, align='left')
            y += 0.44
            if subtitle:
                _h2_text(slide, subtitle, left=cx + _LY.PAD, top=y,
                         width=card_w - _LY.PAD * 2, height=0.26,
                         font=font, size_pt=9, color='777777', bold=False, align='left')
            ...
            if body_src:
                _h2_text(slide, body_src, left=cx + _LY.PAD, top=y,
            ...
            if sv:
                _h2_rect(slide, left=cx + _LY.PAD, top=_LY.CB - 0.74,
                         width=card_w - _LY.PAD * 2, height=0.025, color='DDDDDD')
                _h2_text(slide, sv, left=cx + _LY.PAD, top=_LY.CB - 0.70,
                         width=card_w - _LY.PAD * 2, height=0.38,
                         font=font, size_pt=24, color=color, bold=True, align='left')
                if sl:
                    _h2_text(slide, sl, left=cx + _LY.PAD, top=_LY.CB - 0.30,
                             width=card_w - _LY.PAD * 2, height=0.22,
                             font=font, size_pt=8, color='888888', bold=True, align='left')

        elif v == 1:
            # En-tête pleine couleur (36%) + corps clair en bas
            hdr_h = card_h * 0.36
            _h2_rect(slide, left=cx, top=_LY.CT, width=card_w, height=hdr_h, color=color)
            _h2_card_bg(slide, cx, _LY.CT + hdr_h, card_w, card_h - hdr_h, tp, i)
            y_hdr = _LY.CT + 0.16
            if icon:
                _h2_text(slide, icon, left=cx + _LY.PAD, top=y_hdr,
                         width=0.48, height=0.40,
                         font=font, size_pt=22, color='FFFFFF', bold=False, align='left')
            if label:
                _h2_text(slide, label, left=cx + _LY.PAD + 0.50, top=y_hdr + 0.10,
                         width=card_w - _LY.PAD - 0.56, height=0.22,
                         font=font, size_pt=7, color='FFFFFF', bold=True, align='right')
            _h2_text(slide, title,
                     left=cx + _LY.PAD, top=_LY.CT + hdr_h - 0.60,
                     width=card_w - _LY.PAD * 2, height=0.54,
            ...
            # Pilier numéroté: bordure gauche épaisse + grand numéro décoratif + titre + bullets
            r_c = int(color[0:2], 16)
            g_c = int(color[2:4], 16)
            b_c = int(color[4:6], 16)
            tint = (f'{min(255,int(r_c+(255-r_c)*0.88)):02X}'
                    f'{min(255,int(g_c+(255-g_c)*0.88)):02X}'
                    f'{min(255,int(b_c+(255-b_c)*0.88)):02X}')
            _h2_rect(slide, left=cx, top=_LY.CT, width=0.08, height=card_h, color=color)
            _h2_text(slide, str(i + 1),
                     left=cx + card_w - 1.10, top=_LY.CT - 0.06,
                     width=1.05, height=1.28,
                     font=font, size_pt=90, color=tint, bold=True, align='right')
            y = _LY.CT + 0.18
            if icon:
                _h2_text(slide, icon, left=cx + 0.22, top=y,
                         width=0.42, height=0.36,
                         font=font, size_pt=18, color=color, bold=False, align='left')
                y += 0.38
            _h2_text(slide, title, left=cx + 0.22, top=y,
                     width=card_w - 0.30, height=0.46,
                     font=font, size_pt=_LY.T_TITLE + 1, color=dk1, bold=True, align='left')
            y += 0.48
            _h2_rect(slide, left=cx + 0.22, top=y,
                     width=card_w - 0.30, height=0.03, color=color)
            y += 0.12
            if subtitle:
                _h2_text(slide, subtitle, left=cx + 0.22, top=y,
                         width=card_w - 0.30, height=0.26,
                         font=font, size_pt=9, color='777777', bold=False, align='left')
                y += 0.30
            body_h = max(0.10, _LY.CB - y - stat_reserved - 0.06)
            if body_src:
                _h2_text(slide, body_src, left=cx + 0.22, top=y,
                         width=card_w - 0.30, height=body_h,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.2)
            if sv:
                _h2_text(slide, sv, left=cx + 0.22, top=_LY.CB - 0.70,
                         width=card_w - 0.30, height=0.38,
                         font=font, size_pt=24, color=color, bold=True, align='left')
                if sl:
                    _h2_text(slide, sl, left=cx + 0.22, top=_LY.CB - 0.30,
                             width=card_w - 0.30, height=0.22,
                             font=font, size_pt=8, color='888888', bold=True, align='left')

        elif v == 3:
            # v3: bordered card + icon badge + title + items
            r_c3 = int(color[0:2], 16)
            g_c3 = int(color[2:4], 16)
            b_c3 = int(color[4:6], 16)
            tint3 = (f'{min(255,int(r_c3+(255-r_c3)*0.92)):02X}'
                     f'{min(255,int(g_c3+(255-g_c3)*0.92)):02X}'
                     f'{min(255,int(b_c3+(255-b_c3)*0.92)):02X}')
            _h2_rounded_rect(slide, cx, _LY.CT, card_w, card_h, tint3, _LY.R_SM)
            _h2_rect(slide, cx, _LY.CT, 0.07, card_h, color)
            _h2_rect(slide, cx + _LY.R_SM, _LY.CT, 0.07 - _LY.R_SM, card_h, color)
            badge_r = 0.30
            bx3 = cx + card_w / 2 - badge_r
            by3 = _LY.CT + 0.20
            _h2_rounded_rect(slide, bx3, by3, badge_r * 2, badge_r * 2, color, badge_r)
            if icon:
                _h2_text(slide, icon, bx3, by3, badge_r * 2, badge_r * 2,
                         font, 14, 'FFFFFF', bold=False, align='center')
            y = _LY.CT + 0.20 + badge_r * 2 + 0.14
            _h2_text(slide, title, left=cx + _LY.PAD, top=y,
                     width=card_w - _LY.PAD * 2, height=0.42,
                     font=font, size_pt=_LY.T_TITLE, color=dk1, bold=True, align='center')
            y += 0.46
            if subtitle:
                _h2_text(slide, subtitle, left=cx + _LY.PAD, top=y,
                         width=card_w - _LY.PAD * 2, height=0.26,
                         font=font, size_pt=9, color='777777', bold=False, align='center')
                y += 0.30
            _h2_rect(slide, left=cx + _LY.PAD, top=y, width=card_w - _LY.PAD * 2,
                     height=0.025, color=color)
            y += 0.10
            body_h3 = max(0.10, _LY.CB - y - stat_reserved - 0.06)
            if body_src:
                _h2_text(slide, body_src, left=cx + _LY.PAD, top=y,
                         width=card_w - _LY.PAD * 2, height=body_h3,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.2)
            if sv:
                _h2_rect(slide, left=cx + _LY.PAD, top=_LY.CB - 0.74,
            ...
            # v4: full-color header card — colored top + ghost number + body below
            hdr4 = card_h * 0.42
            _h2_rounded_rect(slide, cx, _LY.CT, card_w, hdr4, color, _LY.R_SM)
            _h2_rect(slide, cx, _LY.CT + hdr4 - _LY.R_SM, card_w, _LY.R_SM, color)
            r_c4 = int(color[0:2], 16)
            g_c4 = int(color[2:4], 16)
            b_c4 = int(color[4:6], 16)
            tint4 = (f'{min(255,int(r_c4+(255-r_c4)*0.90)):02X}'
                     f'{min(255,int(g_c4+(255-g_c4)*0.90)):02X}'
                     f'{min(255,int(b_c4+(255-b_c4)*0.90)):02X}')
            _h2_rect(slide, cx, _LY.CT + hdr4, card_w, card_h - hdr4, tint4)
            # Ghost number watermark
            _h2_text(slide, str(i + 1),
                     left=cx + card_w - 0.90, top=_LY.CT + 0.04,
                     width=0.82, height=1.00,
                     font=font, size_pt=72, color='FFFFFF', bold=True, align='right')
            _set_shape_alpha(slide.shapes[-1], 20)
            y_h4 = _LY.CT + 0.12
            if icon:
                _h2_text(slide, icon, left=cx + _LY.PAD, top=y_h4,
                         width=0.44, height=0.40,
                         font=font, size_pt=20, color='FFFFFF', bold=False, align='left')
                y_h4 += 0.42
            _h2_text(slide, title,
                     left=cx + _LY.PAD, top=_LY.CT + hdr4 - 0.56,
                     width=card_w - _LY.PAD * 2, height=0.50,
                     font=font, size_pt=_LY.T_TITLE, color='FFFFFF', bold=True, align='left')
            y4 = _LY.CT + hdr4 + 0.14
            if subtitle:
                _h2_text(slide, subtitle, left=cx + _LY.PAD, top=y4,
                         width=card_w - _LY.PAD * 2, height=0.26,
                         font=font, size_pt=9, color='555555', bold=False, align='left')
                y4 += 0.30
            body_h4 = max(0.10, _LY.CB - y4 - stat_reserved - 0.06)
            if body_src:
                _h2_text(slide, body_src, left=cx + _LY.PAD, top=y4,
                         width=card_w - _LY.PAD * 2, height=body_h4,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.2)
            if sv:
                _h2_rect(slide, left=cx + _LY.PAD, top=_LY.CB - 0.74,
...
def layout_infographic_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 3:
        # v3: Bande héro en haut + tuiles de barres horizontales en bas
        hero_h    = (_LY.CB - _LY.CT) * 0.36
        tile_y    = _LY.CT + hero_h + _LY.GAP_SM
        tile_zone = _LY.CB - tile_y
        n_bars    = min(len(bars), 5)
        tile_h    = (tile_zone - _LY.GAP_XS * max(n_bars - 1, 0)) / max(n_bars, 1)
        _h2_rounded_rect(slide, _LY.CL, _LY.CT, _LY.CW, hero_h, 'F0F4F8', _LY.R_SM)
        _h2_rect(slide, _LY.CL, _LY.CT, _LY.CW, 0.06, accent1)
        _h2_text(slide, value,
                 _LY.CL, _LY.CT + 0.10, _LY.CW, hero_h * 0.68,
                 font, 52, accent1, bold=True, align='center')
        if label:
            _h2_text(slide, label,
                     _LY.CL, _LY.CT + hero_h * 0.72, _LY.CW, hero_h * 0.26,
                     font, 13, dk1, bold=False, align='center')
        for i, b in enumerate(bars[:n_bars]):
            bl    = b.get('label', '') if isinstance(b, dict) else str(b)
            pct   = float(b.get('percent', b.get('pct', 50)) if isinstance(b, dict) else 50) / 100
            pct   = max(0.0, min(1.0, pct))
            color = b.get('color', accents[i % len(accents)]) if isinstance(b, dict) else accents[i % len(accents)]
            ty    = tile_y + i * (tile_h + _LY.GAP_XS)
            _h2_rounded_rect(slide, _LY.CL, ty, _LY.CW, tile_h, 'F8F8F8', _LY.R_SM)
            _h2_rounded_rect(slide, _LY.CL, ty, 0.28, tile_h, color, _LY.R_SM)
            _h2_text(slide, bl, _LY.CL + 0.38, ty, _LY.CW * 0.42, tile_h,
                     font, 10, dk1, bold=False, align='left')
            bar_bh = min(0.16, tile_h * 0.35)
            bar_by = ty + (tile_h - bar_bh) / 2
            bar_x  = _LY.CL + _LY.CW * 0.46
            bar_w  = _LY.CW * 0.42
            _h2_rect(slide, bar_x, bar_by, bar_w, bar_bh, 'EEEEEE')
            _h2_rect(slide, bar_x, bar_by, max(0.06, bar_w * pct), bar_bh, color)
            _h2_text(slide, f'{int(pct * 100)}%',
                     _LY.CL + _LY.CW * 0.90, ty,
                     _LY.CR - (_LY.CL + _LY.CW * 0.90) - 0.04, tile_h,
                     font, 11, color, bold=True, align='right')
        return slide

    if v == 4:
        # v4: Carte héro gauche + grille de cartes métriques droite (2 colonnes)
        hero_w  = _LY.CW * 0.34
        grid_x  = _LY.CL + hero_w + _LY.GAP_LG
        grid_w  = _LY.CR - grid_x
        n_bars  = min(len(bars), 6)
        n_cols  = 2
        n_rows  = (n_bars + n_cols - 1) // n_cols
        gap     = _LY.GAP_SM
        card_w  = (grid_w - gap) / n_cols
        card_h  = (_LY.CB - _LY.CT - gap * (n_rows - 1)) / max(n_rows, 1)
        _h2_rounded_rect(slide, _LY.CL, _LY.CT, hero_w, _LY.CB - _LY.CT, 'F0F4F8', _LY.R_SM)
        _h2_rect(slide, _LY.CL, _LY.CT, hero_w, 0.07, accent1)
        y_mid_h = (_LY.CT + _LY.CB) / 2
        _h2_text(slide, value,
                 _LY.CL, y_mid_h - 0.70, hero_w, 1.30,
                 font, 44, accent1, bold=True, align='center')
        if label:
            _h2_text(slide, label,
                     _LY.CL, y_mid_h + 0.64, hero_w, 0.38,
                     font, 12, dk1, bold=False, align='center')
        if context:
            _h2_text(slide, context,
                     _LY.CL + 0.12, y_mid_h + 1.06, hero_w - 0.24, 0.80,
                     font, 9, '666666', bold=False, align='center', line_spacing=1.3)
        for i, b in enumerate(bars[:n_bars]):
            bl    = b.get('label', '') if isinstance(b, dict) else str(b)
            pct   = float(b.get('percent', b.get('pct', 50)) if isinstance(b, dict) else 50) / 100
            pct   = max(0.0, min(1.0, pct))
            color = b.get('color', accents[i % len(accents)]) if isinstance(b, dict) else accents[i % len(accents)]
            cx    = grid_x + (i % n_cols) * (card_w + gap)
            cy    = _LY.CT + (i // n_cols) * (card_h + gap)
            _h2_rounded_rect(slide, cx, cy, card_w, card_h, 'F8F8F8', _LY.R_SM)
            _h2_rect(slide, cx, cy, card_w, 0.06, color)
            _h2_text(slide, f'{int(pct * 100)}%',
                     cx, cy + 0.10, card_w, card_h * 0.55,
                     font, 28, color, bold=True, align='center')
            _h2_text(slide, bl,
                     cx + 0.08, cy + card_h * 0.60, card_w - 0.16, card_h * 0.24,
                     font, 9, dk1, bold=False, align='center', line_spacing=1.1)
            pb_y = cy + card_h - 0.16
            pb_w = card_w - 0.16
            _h2_rect(slide, cx + 0.08, pb_y, pb_w, 0.10, 'DDDDDD')
            _h2_rect(slide, cx + 0.08, pb_y, max(0.06, pb_w * pct), 0.10, color)
        return slide
...
def layout_timeline_v4(prs: Presentation, content: dict, tp: dict):
    """
    Frise chronologique — 3 variantes visuelles déterministes.
    v0 : frise horizontale, jalons alternés dessus/dessous.
    v1 : frise verticale, ligne accent1 à gauche, blocs à droite.
    v2 : cartes colorées horizontales avec numéro grand + date pill + contenu.
    """
    slide  = _blank_v4(prs, tp)
    font   = tp.get('font', 'Calibri')
    theme  = tp.get('theme', {})
    ...
    if v == 0:
        ...
        gap_x   = _LY.GAP_SM
        gap_y   = _LY.GAP_MD
        card_w  = (_LY.CW - gap_x * (n_cols - 1)) / n_cols
        ...
        for i, step in enumerate(steps[:n]):
            ...
            # Card background
            bg = _cbg(tp, 0)
            _h2_rounded_rect(slide, cx, cy, card_w, card_h, bg, _LY.R_SM)
            # Colored top band
            _h2_rect(slide, cx, cy, card_w, 0.08, color)
            # Large decorative number bottom-right
            _h2_text(slide, str(i + 1),
                     left=cx + card_w * 0.52, top=cy + card_h * 0.08,
                     width=card_w * 0.46, height=card_h * 0.75,
                     font=font, size_pt=72, color=color, bold=True, align='right')
            # Date pill
            y_txt = cy + 0.16
            if date:
                dw = min(card_w - 0.20, len(date) * 0.11 + 0.40)
                _h2_rounded_rect(slide, cx + 0.10, y_txt, dw, 0.26, color, 0.13)
                _h2_text(slide, date, cx + 0.10, y_txt, dw, 0.26,
                         font, 8, 'FFFFFF', bold=True, align='center')
                y_txt += 0.34
            # Title
            _h2_text(slide, title,
                     left=cx + 0.10, top=y_txt,
                     width=card_w * 0.62, height=min(0.46, card_h - (y_txt - cy) - 0.10),
                     font=font, size_pt=12, color=dk1, bold=True, align='left',
                     line_spacing=1.1)
            if body:
                _h2_text(slide, body,
                         left=cx + 0.10, top=y_txt + 0.48,
                         width=card_w * 0.62, height=card_h - (y_txt - cy) - 0.58,
...
def layout_funnel_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: Pyramide de cartes centrées — largeur décroissante proportionnelle au rang
        total_h = _LY.CB - _LY.CT
        lvl_h   = total_h / n - _LY.GAP_XS
        max_w   = _LY.CW
        for i, step in enumerate(steps[:n]):
            label, value, pct, delta = _sd(step)
            try:
                ratio = max(0.25, float(pct) / 100) if pct is not None else \
                        1.0 - i * (1.0 - 0.25) / max(n - 1, 1)
            except (ValueError, TypeError):
                ratio = 1.0 - i * (1.0 - 0.25) / max(n - 1, 1)
            lw    = max_w * ratio
            lx    = _LY.CL + (max_w - lw) / 2
            ly    = _LY.CT + i * (lvl_h + _LY.GAP_XS)
            color = accents[i % len(accents)]
            _h2_rounded_rect(slide, lx, ly, lw, lvl_h, color, _LY.R_SM)
            cy2 = ly + (lvl_h - 0.34) / 2
            _h2_rounded_rect(slide, lx + 0.12, cy2 + 0.02, 0.28, 0.28, _darken(color, 0.75), 0.14)
            _h2_text(slide, str(i + 1), lx + 0.12, cy2 + 0.02, 0.28, 0.28,
                     font, 9, 'FFFFFF', bold=True, align='center')
            _h2_text(slide, label, lx + 0.48, cy2, lw * 0.50, 0.34,
                     font, 11, 'FFFFFF', bold=True, align='left')
            if value:
                _h2_text(slide, value, lx + lw * 0.62, cy2, lw * 0.26, 0.34,
                         font, 14, 'FFFFFF', bold=True, align='right')
            if delta:
                _h2_text(slide, delta, lx + lw * 0.89, cy2, lw * 0.10, 0.34,
                         font, 9, 'FFFFFF', bold=False, align='center')

    elif v == 4:
        # v4: Dashboard liste — badge rang + label + barre proportionnelle + valeur + delta
        total_h = _LY.CB - _LY.CT
        lvl_h   = total_h / n - _LY.GAP_XS
        lbl_x   = _LY.CL + 0.50
        lbl_w   = _LY.CW * 0.34
        bar_x   = lbl_x + lbl_w + 0.12
        val_w   = 1.30
        dlt_w   = 0.80
        max_bar = _LY.CR - bar_x - val_w - dlt_w - 0.10
        for i, step in enumerate(steps[:n]):
            label, value, pct, delta = _sd(step)
            ly    = _LY.CT + i * (lvl_h + _LY.GAP_XS)
            color = accents[i % len(accents)]
            try:
                ratio = max(0.05, float(pct) / 100) if pct is not None else \
                        max(0.05, 1.0 - i * 0.18 / max(n - 1, 1))
            except (ValueError, TypeError):
                ratio = max(0.05, 1.0 - i * 0.18 / max(n - 1, 1))
            _h2_rounded_rect(slide, _LY.CL, ly, _LY.CW, lvl_h, 'F4F7FA', _LY.R_SM)
            _h2_rect(slide, _LY.CL, ly, 0.06, lvl_h, color)
            _h2_rounded_rect(slide, _LY.CL + 0.10, ly + (lvl_h - 0.28) / 2,
                             0.28, 0.28, color, 0.14)
            _h2_text(slide, str(i + 1), _LY.CL + 0.10, ly + (lvl_h - 0.28) / 2,
                     0.28, 0.28, font, 9, 'FFFFFF', bold=True, align='center')
            cy2 = ly + (lvl_h - 0.28) / 2
            _h2_text(slide, label, lbl_x, cy2, lbl_w, 0.28,
                     font, 11, dk1, bold=True, align='left')
            bh  = min(0.14, lvl_h * 0.34)
            bby = ly + (lvl_h - bh) / 2
            _h2_rect(slide, bar_x, bby, max_bar, bh, 'EEEEEE')
            _h2_rect(slide, bar_x, bby, max(0.06, max_bar * ratio), bh, color)
            if value:
                _h2_text(slide, value, bar_x + max_bar + 0.08, cy2, val_w, 0.28,
                         font, 12, color, bold=True, align='center')
            if delta:
                _h2_text(slide, delta, bar_x + max_bar + val_w + 0.10, cy2, dlt_w, 0.28,
...
def layout_pyramid_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: Bandes rectangulaires centrées — gradient d'accent, sans MSO shapes
        zone_h  = _LY.CB - _LY.CT
        level_h = zone_h / n - 0.05
        min_w   = _LY.CW * 0.18
        max_w   = _LY.CW * 0.96
        for i, level in enumerate(levels[:n]):
            ...
            ly    = _LY.CT + i * (level_h + 0.05)
            color = shade(i, n)
            _h2_rounded_rect(slide, lx, ly, lw, level_h, color, _LY.R_SM)
            cy2   = ly + (level_h - 0.32) / 2
            _h2_text(slide, label,
                     left=lx + 0.12, top=cy2, width=lw * 0.48, height=0.32,
                     font=font, size_pt=11, color='FFFFFF', bold=True, align='left')
            if body:
                _h2_text(slide, body,
                         left=lx + lw * 0.52, top=cy2, width=lw * 0.46, height=0.32,
                         font=font, size_pt=9, color='FFFFFF', bold=False, align='left')

    elif v == 4:
        # v4: Cartes de niveaux empilées — badge, numéro fantôme, label + corps
        zone_h  = _LY.CB - _LY.CT
        level_h = zone_h / n - _LY.GAP_XS
        for i, level in enumerate(levels[:n]):
            label = level.get('label', '') if isinstance(level, dict) else str(level)
            body  = level.get('body', '')  if isinstance(level, dict) else ''
            color = shade(i, n)
            ...
            r_c, g_c, b_c = int(color[0:2], 16), int(color[2:4], 16), int(color[4:6], 16)
            ghost = (f'{min(255,int(r_c+(255-r_c)*0.82)):02X}'
                     f'{min(255,int(g_c+(255-g_c)*0.82)):02X}'
                     f'{min(255,int(b_c+(255-b_c)*0.82)):02X}')
            _h2_rounded_rect(slide, _LY.CL, ly, _LY.CW, level_h, 'F4F7FA', _LY.R_SM)
            _h2_rect(slide, _LY.CL, ly, 0.06, level_h, color)
            _h2_text(slide, str(i + 1),
                     left=_LY.CR - 0.78, top=ly + 0.02,
                     width=0.74, height=level_h * 0.72,
                     font=font, size_pt=36, color=ghost, bold=True, align='right')
            _h2_rounded_rect(slide, _LY.CL + 0.10, ly + (level_h - 0.28) / 2, 0.28, 0.28, color, 0.14)
            _h2_text(slide, str(i + 1), _LY.CL + 0.10, ly + (level_h - 0.28) / 2, 0.28, 0.28,
                     font, 9, 'FFFFFF', bold=True, align='center')
            tx = _LY.CL + 0.48
            tw = _LY.CW - 1.06
            if body:
                ty = ly + level_h * 0.12
                _h2_text(slide, label, tx, ty, tw * 0.44, min(0.34, level_h * 0.44),
                         font, 11, dk1, bold=True, align='left')
                _h2_text(slide, body, tx + tw * 0.46, ty, tw * 0.52,
                         min(0.34, level_h * 0.44),
                         font, 9, '555555', bold=False, align='left', line_spacing=1.1)
            else:
                _h2_text(slide, label, tx, ly + (level_h - 0.30) / 2, tw, 0.30,
                         font, 11, dk1, bold=True, align='left')
...
def layout_cycle_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: Grille cyclique 2 rangées — haut gauche→droite, bas droite→gauche
        n_top  = (n + 1) // 2
        n_bot  = n - n_top
        arr_w  = 0.34
        zone_h = _LY.CB - _LY.CT
        row_h  = zone_h * 0.38
        gap_y  = zone_h - row_h * 2
        top_y  = _LY.CT
        bot_y  = _LY.CT + row_h + gap_y
        c_w1   = (_LY.CW - arr_w * max(n_top - 1, 0)) / max(n_top, 1)
        for j in range(n_top):
            step   = steps[j]
            color  = accents[j % len(accents)]
            t_text = step.get('title', '') if isinstance(step, dict) else str(step)
            b_text = step.get('body',  '') if isinstance(step, dict) else ''
            bx     = _LY.CL + j * (c_w1 + arr_w)
            _h2_rounded_rect(slide, bx, top_y, c_w1, row_h, color, _LY.R_SM)
            _h2_text(slide, str(j + 1), bx, top_y + row_h * 0.05,
                     c_w1, row_h * 0.30, font, 16, 'FFFFFF', bold=True, align='center')
            _h2_text(slide, t_text, bx + 0.10, top_y + row_h * 0.36,
                     c_w1 - 0.20, row_h * 0.44,
                     font, 9, 'FFFFFF', bold=True, align='center', line_spacing=1.1)
            if b_text:
                _h2_text(slide, b_text, bx + 0.10, top_y + row_h * 0.80,
                         c_w1 - 0.20, row_h * 0.17,
                         font, 7, 'FFFFFF', bold=False, align='center')
            if j < n_top - 1:
                _h2_autoshape(slide, 13,
                              bx + c_w1 + 0.03, top_y + (row_h - 0.22) / 2,
                              arr_w - 0.06, 0.22, accents[(j + 1) % len(accents)])
        if n_bot > 0:
            c_w2 = (_LY.CW - arr_w * max(n_bot - 1, 0)) / max(n_bot, 1)
            for j in range(n_bot):
                step_i = n_top + j
                step   = steps[step_i]
                color  = accents[step_i % len(accents)]
                t_text = step.get('title', '') if isinstance(step, dict) else str(step)
                b_text = step.get('body',  '') if isinstance(step, dict) else ''
                bx     = _LY.CR - (j + 1) * c_w2 - j * arr_w
                _h2_rounded_rect(slide, bx, bot_y, c_w2, row_h, color, _LY.R_SM)
                _h2_text(slide, str(step_i + 1), bx, bot_y + row_h * 0.05,
                         c_w2, row_h * 0.30, font, 16, 'FFFFFF', bold=True, align='center')
                _h2_text(slide, t_text, bx + 0.10, bot_y + row_h * 0.36,
                         c_w2 - 0.20, row_h * 0.44,
                         font, 9, 'FFFFFF', bold=True, align='center', line_spacing=1.1)
                if b_text:
                    _h2_text(slide, b_text, bx + 0.10, bot_y + row_h * 0.80,
                             c_w2 - 0.20, row_h * 0.17,
                             font, 7, 'FFFFFF', bold=False, align='center')
                if j < n_bot - 1:
                    ax = bx - arr_w + 0.03
                    _h2_autoshape(slide, 13, ax, bot_y + (row_h - 0.22) / 2,
                                  arr_w - 0.06, 0.22,
                                  accents[(step_i + 1) % len(accents)], rotation=180)
            _h2_autoshape(slide, 36, _LY.CR - 0.22, top_y + row_h + 0.04,
                          0.22, gap_y - 0.08, accents[n_top % len(accents)])
            ret_h = _LY.CB - bot_y - row_h
            if ret_h > 0.14:
                _h2_autoshape(slide, 36, _LY.CL, bot_y + row_h + (ret_h - 0.28) / 2,
                              0.22, 0.28, accents[0], rotation=180)

    elif v == 4:
        # v4: Cycle liste verticale — ligne connecteur verticale + flèche retour
        line_x  = _LY.CL + 0.30
        dot_r   = 0.16
        row_gap = _LY.GAP_XS
        row_h   = (_LY.CB - _LY.CT - row_gap * (n - 1)) / max(n, 1)
        card_x  = line_x + dot_r + 0.28
        card_w  = _LY.CR - card_x - 0.05
        _h2_rect(slide, left=line_x - 0.018, top=_LY.CT + dot_r,
                 width=0.036, height=_LY.CB - _LY.CT - dot_r * 2, color='DDDDDD')
        _h2_autoshape(slide, 36, line_x - 0.14, _LY.CB - 0.36, 0.28, 0.36,
                      accents[0], rotation=180)
        for i, step in enumerate(steps[:n]):
            t_text = step.get('title', '') if isinstance(step, dict) else str(step)
            b_text = step.get('body',  '') if isinstance(step, dict) else ''
            color  = accents[i % len(accents)]
            ...
            if b_text:
                ty = ry + row_h * 0.12
                _h2_text(slide, t_text, card_x + 0.10, ty, card_w - 0.20,
                         min(0.34, row_h * 0.44),
                         font, 11, dk1, bold=True, align='left', line_spacing=1.1)
                _h2_text(slide, b_text, card_x + 0.10, ty + 0.36, card_w - 0.20,
                         max(0.22, row_h * 0.88 - 0.46),
                         font, 9, '555555', align='left', line_spacing=1.15)
            else:
                _h2_text(slide, t_text, card_x + 0.10, ry + (row_h - 0.30) / 2,
                         card_w - 0.20, 0.30, font, 11, dk1,
                         bold=True, align='left', line_spacing=1.1)
...
def layout_roadmap_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: swimlane — bandes horizontales par phase + jalons en colonnes
        lane_h = (_LY.CB - _LY.CT) / max(n, 1)
        for i, phase in enumerate(phases[:n]):
            label      = phase.get('label', '') if isinstance(phase, dict) else str(phase)
            milestones = phase.get('milestones', []) if isinstance(phase, dict) else []
            color      = accents[i % len(accents)]
            ...
            g_c3 = int(color[2:4], 16)
            b_c3 = int(color[4:6], 16)
            tint3 = (f'{min(255,int(r_c3+(255-r_c3)*0.90)):02X}'
                     f'{min(255,int(g_c3+(255-g_c3)*0.90)):02X}'
                     f'{min(255,int(b_c3+(255-b_c3)*0.90)):02X}')
            _h2_rect(slide, left=_LY.CL, top=ly, width=_LY.CW, height=lane_h - 0.04, color=tint3)
            _h2_rect(slide, left=_LY.CL, top=ly, width=0.06, height=lane_h - 0.04, color=color)
            lbl_w = 1.80
            _h2_text(slide, label, left=_LY.CL + 0.14, top=ly + (lane_h - 0.04 - 0.28) / 2,
                     width=lbl_w - 0.14, height=0.28,
                     font=font, size_pt=11, color=color, bold=True, align='left')
            n_ms   = min(len(milestones), 5)
            ms_w   = (_LY.CW - lbl_w) / max(n_ms, 1)
            for j, ms in enumerate(milestones[:n_ms]):
                mx = _LY.CL + lbl_w + j * ms_w + 0.08
                _h2_text(slide, str(ms),
                         left=mx, top=ly + 0.06,
                         width=ms_w - 0.16, height=lane_h - 0.16,
                         font=font, size_pt=9, color=dk1,
                         bold=False, align='left', line_spacing=1.1)

    elif v == 4:
        # v4: roadmap verticale avec indicateurs de progression (barres)
        row_h = (_LY.CB - _LY.CT - _LY.GAP_XS * (n - 1)) / max(n, 1)
        prog_vals = [0.25, 0.50, 0.80, 1.0, 0.10]
        for i, phase in enumerate(phases[:n]):
            label      = phase.get('label', '') if isinstance(phase, dict) else str(phase)
            milestones = phase.get('milestones', []) if isinstance(phase, dict) else []
            color      = accents[i % len(accents)]
            ry         = _LY.CT + i * (row_h + _LY.GAP_XS)
            _h2_rounded_rect(slide, _LY.CL, ry, _LY.CW, row_h, 'F4F7FA', _LY.R_SM)
            # Phase number badge
            _h2_rounded_rect(slide, _LY.CL + 0.10, ry + (row_h - 0.42) / 2,
                             0.42, 0.42, color, 0.21)
            _h2_text(slide, str(i + 1),
                     left=_LY.CL + 0.10, top=ry + (row_h - 0.42) / 2,
                     width=0.42, height=0.42,
                     font=font, size_pt=14, color='FFFFFF', bold=True, align='center')
            # Label
            lbl_w4 = 2.40
            _h2_text(slide, label, left=_LY.CL + 0.64, top=ry + (row_h - 0.28) / 2,
                     width=lbl_w4, height=0.28,
                     font=font, size_pt=11, color=dk1, bold=True, align='left')
            # Milestones as tags
            tags_x = _LY.CL + 0.64 + lbl_w4 + 0.10
            tags_w = _LY.CR - tags_x - 1.30
            n_ms = min(len(milestones), 3)
            tag_w = (tags_w - _LY.GAP_XS * (n_ms - 1)) / max(n_ms, 1)
            for j, ms in enumerate(milestones[:n_ms]):
                tx = tags_x + j * (tag_w + _LY.GAP_XS)
                _h2_rounded_rect(slide, tx, ry + (row_h - 0.30) / 2, tag_w, 0.30, 'DDDDDD', 0.08)
                _h2_text(slide, str(ms), left=tx, top=ry + (row_h - 0.28) / 2,
                         width=tag_w, height=0.28,
                         font=font, size_pt=8, color='555555', bold=False, align='center')
            # Progress bar
            prog = phase.get('progress', prog_vals[i % len(prog_vals)])
            bar_w = 1.10
            bar_x = _LY.CR - bar_w - 0.10
            bar_h = 0.14
            bar_y = ry + (row_h - bar_h) / 2
            _h2_rounded_rect(slide, bar_x, bar_y, bar_w, bar_h, 'DDDDDD', 0.07)
            if prog > 0:
                _h2_rounded_rect(slide, bar_x, bar_y, max(bar_h, bar_w * prog), bar_h, color, 0.07)
            _h2_text(slide, f'{int(prog*100)}%',
                     left=bar_x - 0.36, top=bar_y - 0.02,
                     width=0.34, height=0.18,
                     font=font, size_pt=8, color=color, bold=True, align='right')
...
def layout_beforeafter_v4(prs: Presentation, content: dict, tp: dict):
    """
    Avant / Après — 3 variantes visuelles déterministes.
    v0 : deux colonnes header coloré + fond palette + flèche centrale.
    v1 : colonne gauche EEEEEE + colonne droite teinté, items avec icône ✗/✓.
    v2 : panneau plein couleur gauche (gris, AVANT) + flèche MSO + panneau accent (APRÈS).
    ...
    if v == 0:
        ...
        # Variante 2 : deux panneaux pleins couleur séparés par une flèche MSO large
        pan_gap = 0.72
        pan_w   = (_LY.CW - pan_gap) / 2
        pan_h   = _LY.CB - _LY.CT
        bef_x   = _LY.CL
        aft_x   = _LY.CL + pan_w + pan_gap
        arr_cx  = _LY.CL + pan_w + pan_gap / 2

        # Avant panel — gris neutre
        _h2_rounded_rect(slide, bef_x, _LY.CT, pan_w, pan_h, '888888', _LY.R_SM)
        # Après panel — accent
        _h2_rounded_rect(slide, aft_x, _LY.CT, pan_w, pan_h, accent1, _LY.R_SM)

        # Central MSO right arrow
        arr_h = 0.55
        arr_w = pan_gap - 0.08
        _h2_autoshape(slide, 13,
                      arr_cx - arr_w / 2, (_LY.CT + _LY.CB) / 2 - arr_h / 2,
                      arr_w, arr_h, accent1)

        for (cx, col, text_color, lbl) in [
            (bef_x, before, 'FFFFFF', 'AVANT'),
            (aft_x,  after,  'FFFFFF', 'APRÈS'),
        ]:
            col_title = col.get('title', lbl) if isinstance(col, dict) else lbl
            items     = col.get('items', []) if isinstance(col, dict) else []
            _h2_text(slide, col_title,
                     left=cx + _LY.PAD, top=_LY.CT + 0.14,
                     width=pan_w - _LY.PAD * 2, height=0.44,
                     font=font, size_pt=16, color=text_color, bold=True, align='left')
            _h2_rect(slide, cx + _LY.PAD, _LY.CT + 0.60,
                     pan_w * 0.25, 0.04, 'FFFFFF')
            for j, item in enumerate(items[:6]):
                iy = _LY.CT + 0.76 + j * 0.54
                if iy + 0.44 > _LY.CB:
                    break
                _h2_text(slide, f'• {item}',
                         left=cx + _LY.PAD, top=iy,
                         width=pan_w - _LY.PAD * 2, height=0.44,
                         font=font, size_pt=11, color=text_color,
                         bold=False, align='left', line_spacing=1.1)

    elif v == 3:
        # v3: diagonal split — avant gauche avec fond accent atténué + après droite accent1 plein
        diag_x  = _LY.CL + _LY.CW * 0.48
        pan_h   = _LY.CB - _LY.CT
        r_ba3   = int(accent1[0:2], 16)
        g_ba3   = int(accent1[2:4], 16)
        b_ba3   = int(accent1[4:6], 16)
        tint_ba = (f'{min(255,int(r_ba3+(255-r_ba3)*0.85)):02X}'
                   f'{min(255,int(g_ba3+(255-g_ba3)*0.85)):02X}'
                   f'{min(255,int(b_ba3+(255-b_ba3)*0.85)):02X}')
        _h2_rounded_rect(slide, _LY.CL, _LY.CT, diag_x - _LY.CL + 0.20, pan_h, tint_ba, _LY.R_SM)
        _h2_rounded_rect(slide, diag_x - 0.20, _LY.CT, _LY.CR - diag_x + 0.20, pan_h, accent1, _LY.R_SM)
        bef_title = before.get('title', 'AVANT') if isinstance(before, dict) else 'AVANT'
        aft_title = after.get('title', 'APRÈS') if isinstance(after, dict) else 'APRÈS'
        bef_items3 = before.get('items', []) if isinstance(before, dict) else []
        aft_items3 = after.get('items',  []) if isinstance(after,  dict) else []
        # Before
        _h2_text(slide, '✗', left=_LY.CL + 0.14, top=_LY.CT + 0.12,
                 width=0.36, height=0.36, font=font, size_pt=16, color=accent1, bold=True, align='center')
        _h2_text(slide, bef_title, left=_LY.CL + 0.55, top=_LY.CT + 0.14,
                 width=diag_x - _LY.CL - 0.60, height=0.32,
                 font=font, size_pt=13, color=dk1, bold=True, align='left')
        _h2_rect(slide, _LY.CL + 0.14, _LY.CT + 0.50, diag_x - _LY.CL - 0.20, 0.03, accent1)
        for j, item in enumerate(bef_items3[:6]):
            iy = _LY.CT + 0.62 + j * 0.50
            if iy + 0.44 > _LY.CB: break
            _h2_text(slide, f'• {item}', left=_LY.CL + 0.14, top=iy,
                     width=diag_x - _LY.CL - 0.20, height=0.44,
                     font=font, size_pt=10, color=dk1, bold=False, align='left', line_spacing=1.1)
        # After
        _h2_text(slide, '✓', left=diag_x + 0.10, top=_LY.CT + 0.12,
                 width=0.36, height=0.36, font=font, size_pt=16, color='FFFFFF', bold=True, align='center')
        _h2_text(slide, aft_title, left=diag_x + 0.52, top=_LY.CT + 0.14,
                 width=_LY.CR - diag_x - 0.60, height=0.32,
                 font=font, size_pt=13, color='FFFFFF', bold=True, align='left')
        _h2_rect(slide, diag_x + 0.10, _LY.CT + 0.50, _LY.CR - diag_x - 0.16, 0.03, 'FFFFFF')
        _set_shape_alpha(slide.shapes[-1], 60)
        for j, item in enumerate(aft_items3[:6]):
            iy = _LY.CT + 0.62 + j * 0.50
            if iy + 0.44 > _LY.CB: break
            _h2_text(slide, f'• {item}', left=diag_x + 0.10, top=iy,
                     width=_LY.CR - diag_x - 0.16, height=0.44,
                     font=font, size_pt=10, color='FFFFFF', bold=False, align='left', line_spacing=1.1)

    elif v == 4:
        # v4: tableau comparatif ligne par ligne — avant à gauche, après à droite
        col_h4  = _LY.CB - _LY.CT
        cw4     = (_LY.CW - 0.60) / 2
        bef_x4  = _LY.CL
        aft_x4  = _LY.CL + cw4 + 0.60
        bef_title4 = before.get('title', 'AVANT') if isinstance(before, dict) else 'AVANT'
        aft_title4 = after.get('title', 'APRÈS') if isinstance(after, dict) else 'APRÈS'
        bef_items4 = before.get('items', []) if isinstance(before, dict) else []
        aft_items4 = after.get('items',  []) if isinstance(after,  dict) else []
        n_rows4 = min(max(len(bef_items4), len(aft_items4)), 6)
        row_h4  = (_LY.CB - _LY.CT - 0.52) / max(n_rows4, 1)
        # Headers
        _h2_rect(slide, bef_x4, _LY.CT, cw4, 0.44, 'EEEEEE')
        _h2_text(slide, bef_title4, left=bef_x4 + _LY.PAD, top=_LY.CT + 0.08,
                 width=cw4 - _LY.PAD * 2, height=0.30,
                 font=font, size_pt=12, color='777777', bold=True, align='left')
        _h2_rect(slide, aft_x4, _LY.CT, cw4, 0.44, accent1)
        _h2_text(slide, aft_title4, left=aft_x4 + _LY.PAD, top=_LY.CT + 0.08,
                 width=cw4 - _LY.PAD * 2, height=0.30,
                 font=font, size_pt=12, color='FFFFFF', bold=True, align='left')
        for j in range(n_rows4):
            ry4 = _LY.CT + 0.44 + j * row_h4
            bg4 = 'F7F7F7' if j % 2 == 0 else 'FFFFFF'
            _h2_rect(slide, bef_x4, ry4, cw4, row_h4 - 0.02, bg4)
            _h2_rect(slide, aft_x4, ry4, cw4, row_h4 - 0.02, bg4)
            if j < len(bef_items4):
                _h2_text(slide, f'✗  {bef_items4[j]}', left=bef_x4 + 0.10, top=ry4 + 0.06,
                         width=cw4 - 0.20, height=row_h4 - 0.10,
                         font=font, size_pt=10, color='888888', bold=False, align='left', line_spacing=1.1)
            if j < len(aft_items4):
                _h2_text(slide, f'✓  {aft_items4[j]}', left=aft_x4 + 0.10, top=ry4 + 0.06,
                         width=cw4 - 0.20, height=row_h4 - 0.10,
                         font=font, size_pt=10, color=dk1, bold=False, align='left', line_spacing=1.1)
...
def layout_entity_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 2:
        # Tableau comparatif — entités en colonnes, critères en lignes avec ✓/–
        n2       = min(n, 4)
        hdr_h    = 0.70
        ...
        for i in range(n2):
            ...
            x_txt, w_txt = cx + 0.10, col_w - 0.20
            if icon:
                ...
                x_txt, w_txt = cx + 0.52, col_w - 0.60
            _h2_text(slide, name, left=x_txt, top=_LY.CT + 0.16,
                     width=w_txt, height=hdr_h - 0.20,
                     font=font, size_pt=11, color='FFFFFF', bold=True, align='left')
        entity_items = [(e.get('items', []) if isinstance(e, dict) else []) for e in entities[:n2]]
        for j, crit in enumerate(criteria):
            ry  = _LY.CT + hdr_h + j * row_h
            _h2_rect(slide, left=_LY.CL, top=ry, width=_LY.CW, height=row_h,
                     color='F0F4F8' if j % 2 == 0 else 'FAFAFA')
            for i in range(n2):
                cx    = _LY.CL + i * col_w
                color = accents[i % len(accents)]
                tick  = '✓' if crit in entity_items[i] else '–'
                _h2_text(slide, tick, left=cx + 0.10, top=ry + (row_h - 0.28) / 2,
                         width=0.26, height=0.28, font=font, size_pt=12,
                         color=color if tick == '✓' else 'BBBBBB', bold=True, align='center')
                _h2_text(slide, crit, left=cx + 0.38, top=ry + (row_h - 0.28) / 2,
                         width=col_w - 0.44, height=0.28, font=font, size_pt=9,
                         color=dk1, bold=False, align='left')

    elif v == 3:
        # Spotlight: première entité en panneau hero pleine hauteur (gauche 42%) + autres en grille (droite)
        feat_w = _LY.CW * 0.42
        rest_x = _LY.CL + feat_w + _LY.GAP_MD
        rest_w = _LY.CR - rest_x
        icon, name, badge, label, items, sv, sl = _ent(entities[0])
        color0 = accents[0]
        _h2_rect(slide, _LY.CL, _LY.CT, feat_w, _LY.CB - _LY.CT, color0)
        y = _LY.CT + 0.24
        if icon:
            _h2_text(slide, icon, _LY.CL + 0.10, y, feat_w - 0.20, 0.62,
                     font=font, size_pt=32, color='FFFFFF', bold=False, align='center')
            y += 0.66
        _h2_text(slide, name, _LY.CL + 0.10, y, feat_w - 0.20, 0.44,
                 font=font, size_pt=16, color='FFFFFF', bold=True, align='center')
        y += 0.46
        if badge:
            _h2_rounded_rect(slide, _LY.CL + (feat_w - 1.50) / 2, y, 1.50, 0.26,
                              _darken(color0, 0.75), 0.08)
            _h2_text(slide, badge, _LY.CL + (feat_w - 1.50) / 2, y + 0.02, 1.50, 0.22,
                     font=font, size_pt=8, color='FFFFFF', bold=True, align='center')
            y += 0.32
        _h2_rect(slide, _LY.CL + 0.20, y, feat_w - 0.40, 0.025, 'FFFFFF')
        y += 0.12
        for it in items[:5]:
            _h2_text(slide, f'• {it}', _LY.CL + 0.20, y, feat_w - 0.30, 0.28,
                     font=font, size_pt=10, color='FFFFFF', bold=False, align='left')
            y += 0.30
        if sv:
            _h2_text(slide, sv, _LY.CL, _LY.CB - 0.82, feat_w, 0.52,
                     font=font, size_pt=28, color='FFFFFF', bold=True, align='center')
            if sl:
                _h2_text(slide, sl, _LY.CL, _LY.CB - 0.28, feat_w, 0.22,
                         font=font, size_pt=8, color='FFFFFF', bold=True, align='center')
        rest_n = min(n - 1, 3)
        if rest_n > 0:
            gap    = _LY.GAP_SM
            card_h = (_LY.CB - _LY.CT - gap * (rest_n - 1)) / rest_n
            for i in range(rest_n):
                eid  = i + 1
                icon, name, badge, label, items, sv, sl = _ent(entities[eid])
                color = accents[eid % len(accents)]
                cy   = _LY.CT + i * (card_h + gap)
                _h2_card_bg(slide, rest_x, cy, rest_w, card_h, tp, i)
                _h2_rect(slide, rest_x, cy, rest_w, 0.055, color)
                y = cy + 0.16
                if icon:
                    _h2_text(slide, icon, rest_x + _LY.PAD, y, 0.38, 0.32,
                             font=font, size_pt=17, color=color, bold=False, align='left')
                    _h2_text(slide, name, rest_x + _LY.PAD + 0.40, y + 0.04,
                             rest_w - _LY.PAD - 0.46, 0.28,
                             font=font, size_pt=11, color=dk1, bold=True, align='left')
                    y += 0.36
                else:
                    _h2_text(slide, name, rest_x + _LY.PAD, y, rest_w - _LY.PAD * 2, 0.28,
                             font=font, size_pt=11, color=dk1, bold=True, align='left')
                    y += 0.30
                if badge:
                    _h2_rounded_rect(slide, rest_x + _LY.PAD, y, 1.10, 0.22, color, 0.06)
                    _h2_text(slide, badge, rest_x + _LY.PAD, y + 0.02, 1.10, 0.18,
                             font=font, size_pt=7, color='FFFFFF', bold=True, align='center')
                    y += 0.28
                n_fit = max(1, int((card_h - (y - cy) - 0.08) / 0.26))
                for it in items[:n_fit]:
                    _h2_text(slide, f'• {it}', rest_x + _LY.PAD, y,
                             rest_w - _LY.PAD * 2, 0.24,
                             font=font, size_pt=9, color=dk1, bold=False, align='left')
                    y += 0.26

    elif v == 4:
        # Scorecard: rangées pleine largeur — panneau entité coloré gauche + items en chips droite
        n4     = min(n, 4)
        gap    = _LY.GAP_XS
        row_h  = (_LY.CB - _LY.CT - gap * (n4 - 1)) / n4
        ent_w  = 2.80
        tags_x = _LY.CL + ent_w + _LY.GAP_MD
        tags_w = _LY.CR - tags_x
        for i in range(n4):
            icon, name, badge, label, items, sv, sl = _ent(entities[i])
            color = accents[i % len(accents)]
            cy    = _LY.CT + i * (row_h + gap)
            r_c, g_c, b_c = int(color[0:2],16), int(color[2:4],16), int(color[4:6],16)
            tint = (f'{min(255,int(r_c+(255-r_c)*0.90)):02X}'
                    f'{min(255,int(g_c+(255-g_c)*0.90)):02X}'
                    f'{min(255,int(b_c+(255-b_c)*0.90)):02X}')
            # Panneau entité (gauche coloré)
            _h2_rounded_rect(slide, _LY.CL, cy, ent_w, row_h, color, _LY.R_SM)
            y_inner = cy + (row_h - (0.78 if icon else 0.38)) / 2
            if icon:
                _h2_text(slide, icon, _LY.CL, y_inner, ent_w, 0.38,
                         font=font, size_pt=20, color='FFFFFF', bold=False, align='center')
                y_inner += 0.40
            _h2_text(slide, name, _LY.CL + 0.10, y_inner, ent_w - 0.20, 0.36,
                     font=font, size_pt=12, color='FFFFFF', bold=True, align='center')
            if sv:
                _h2_text(slide, sv, _LY.CL + 0.10, cy + row_h - 0.34,
                         ent_w - 0.20, 0.28,
                         font=font, size_pt=16, color='FFFFFF', bold=True, align='center')
            # Zone tags (droite)
            _h2_card_bg(slide, tags_x, cy, tags_w, row_h, tp, i)
            _h2_rect(slide, tags_x, cy, 0.04, row_h, color)
            pill_w = min((tags_w - 0.20) / 3 - 0.08, 2.90)
            pill_h = 0.28
            cols   = max(1, int((tags_w - 0.20) / (pill_w + 0.08)))
            for j, it in enumerate(items[:9]):
                col = j % cols
                rw  = j // cols
                px  = tags_x + 0.14 + col * (pill_w + 0.08)
                py  = cy + 0.14 + rw * (pill_h + 0.10)
                if py + pill_h > cy + row_h - 0.08:
                    break
                _h2_rounded_rect(slide, px, py, pill_w, pill_h, tint, 0.08)
                _h2_text(slide, str(it), px + 0.08, py + 0.02, pill_w - 0.12, pill_h - 0.04,
                         font=font, size_pt=9, color=dk1, bold=False, align='left')
...
def layout_conclusion_v4(prs: Presentation, content: dict, tp: dict):
    """
    Slide de synthèse/conclusion.
    v0: grille 2×2 cartes (gauche) + sidebar sombre (droite)
    v1: grille pleine largeur + bande CTA bas pleine couleur
    v2: message central grand format + rangée de cartes en bas
    ...
    if v == 0:
        ...
        for i, card in enumerate(cards):
            ...
            ttl   = card.get('title', '') if isinstance(card, dict) else str(card)
            body  = card.get('body', '') if isinstance(card, dict) else ''
            _h2_card_bg(slide, cx, cy, card_w, card_h, tp, i)
            _h2_rect(slide, left=cx, top=cy, width=0.055, height=card_h, color=color)
            y = cy + 0.14
            if icon:
                _h2_text(slide, icon, left=cx + 0.18, top=y, width=0.38, height=0.36,
                         font=font, size_pt=15, color=dk1, bold=False, align='left')
                y += 0.38
            _h2_text(slide, ttl, left=cx + 0.18, top=y,
                     width=card_w - 0.26, height=0.40,
                     font=font, size_pt=_LY.T_TITLE, color=dk1, bold=True, align='left')
            y += 0.42
            if body:
                _h2_text(slide, body, left=cx + 0.18, top=y,
        ...
        sy = _LY.CT + 0.22
        if sb_title:
            _h2_text(slide, sb_title, left=sidebar_x + 0.20, top=sy,
        ...
        if sb_quote:
            ...
            _h2_text(slide, sb_quote, left=sidebar_x + 0.24, top=sy + 0.12,
        ...
        if sb_cta:
            ...
            _h2_text(slide, sb_cta, left=sidebar_x + 0.14, top=cta_y + 0.38,
                     width=sidebar_w - 0.22, height=0.55,
                     font=font, size_pt=12, color='FFFFFF',
                     bold=True, align='center')

    elif v == 1:
        # Grille pleine largeur (pas de sidebar) + bande CTA colorée en bas
        cta_text = sb_cta or sb_title or ''
        cta_h    = 0.72 if cta_text else 0.0
        grid_top = _LY.CT
        grid_bot = _LY.CB - cta_h - (0.10 if cta_text else 0.0)
        n        = max(len(cards), 1)
        cols     = min(n, 4)
        rows     = (n + cols - 1) // cols
        gap      = _LY.GAP_SM
        card_w   = (_LY.CW - gap * (cols - 1)) / cols
        card_h   = (grid_bot - grid_top - gap * (rows - 1)) / rows
        ...
        for i, card in enumerate(cards):
            ...
            body  = card.get('body', '') if isinstance(card, dict) else ''
            _h2_card_bg(slide, cx, cy, card_w, card_h, tp, i)
            _h2_rect(slide, left=cx, top=cy, width=card_w, height=0.06, color=color)
            y = cy + 0.18
            if icon:
                _h2_text(slide, icon, left=cx + _LY.PAD, top=y, width=0.40, height=0.36,
                         font=font, size_pt=16, color=color, bold=False, align='left')
                y += 0.38
            _h2_text(slide, ttl, left=cx + _LY.PAD, top=y,
                     width=card_w - _LY.PAD * 2, height=0.40,
                     font=font, size_pt=_LY.T_TITLE, color=dk1, bold=True, align='left')
            y += 0.42
            if body:
                _h2_text(slide, body, left=cx + _LY.PAD, top=y,
                         width=card_w - _LY.PAD * 2, height=cy + card_h - y - 0.10,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.2)

        if cta_text:
            _h2_rect(slide, left=_LY.CL, top=grid_bot + 0.10,
                     width=_LY.CW, height=cta_h - 0.10, color=accent1)
            _h2_text(slide, cta_text,
                     left=_LY.CL + 0.30, top=grid_bot + 0.20,
                     width=_LY.CW - 0.60, height=cta_h - 0.30,
                     font=font, size_pt=13, color='FFFFFF',
                     bold=True, align='center')

        if sb_quote and not cta_text:
            _h2_text(slide, f'"{sb_quote}"',
                     left=_LY.CL, top=_LY.CB - 0.55,
                     width=_LY.CW, height=0.44,
                     font=font, size_pt=9, color='888888',
                     bold=False, italic=True, align='center')

    elif v == 2:
        # Message central grand format + rangée de cartes au bas
        n_cards  = len(cards)
        row_h    = 1.80 if n_cards > 0 else 0.0
        msg_bot  = _LY.CB - row_h - (0.12 if n_cards else 0.0)
        msg_text = sb_quote or sb_title or ''

        if msg_text:
            msg_h = msg_bot - _LY.CT - 0.10
            _h2_text(slide, msg_text,
                     left=_LY.CL + 0.30, top=_LY.CT + 0.10,
                     width=_LY.CW - 0.60, height=msg_h,
                     font=font, size_pt=22, color=dk1,
                     bold=False, italic=True, align='center', line_spacing=1.4)
            _h2_rect(slide, left=_LY.CL + _LY.CW * 0.35, top=msg_bot - 0.10,
                     width=_LY.CW * 0.30, height=0.04, color=accent1)

        if n_cards:
            cols   = min(n_cards, 4)
            gap    = _LY.GAP_SM
            c_w    = (_LY.CW - gap * (cols - 1)) / cols
            c_top  = _LY.CB - row_h
            for i, card in enumerate(cards[:cols]):
                cx    = _LY.CL + i * (c_w + gap)
                color = accents[i % len(accents)]
                icon  = card.get('icon', '') if isinstance(card, dict) else ''
                ttl   = card.get('title', '') if isinstance(card, dict) else str(card)
                body  = card.get('body', '') if isinstance(card, dict) else ''
                _h2_card_bg(slide, cx, c_top, c_w, row_h, tp, i)
                _h2_rect(slide, left=cx, top=c_top, width=c_w, height=0.055, color=color)
                y = c_top + 0.16
                if icon:
                    _h2_text(slide, icon, left=cx + _LY.PAD, top=y, width=0.36, height=0.32,
                             font=font, size_pt=14, color=color, bold=False, align='left')
                    y += 0.34
                _h2_text(slide, ttl, left=cx + _LY.PAD, top=y,
                         width=c_w - _LY.PAD * 2, height=0.36,
                         font=font, size_pt=_LY.T_TITLE, color=dk1, bold=True, align='left')
                y += 0.38
                if body:
                    _h2_text(slide, body, left=cx + _LY.PAD, top=y,
                             width=c_w - _LY.PAD * 2, height=c_top + row_h - y - 0.08,
                             font=font, size_pt=_LY.T_SMALL, color=dk1,
                             bold=False, align='left', line_spacing=1.2)

        if sb_cta:
            _h2_text(slide, f'→  {sb_cta}',
                     left=_LY.CL, top=_LY.CT + 0.06,
                     width=_LY.CW, height=0.30,
                     font=font, size_pt=10, color=accent1,
        ...
        gap3   = _LY.GAP_SM
        cw3    = (_LY.CW - gap3 * (cols3 - 1)) / cols3
        ch3    = (_LY.CB - _LY.CT - gap3 * (rows3 - 1)) / rows3
        for i, card in enumerate(cards):
            cx3 = _LY.CL + (i % cols3) * (cw3 + gap3)
            cy3 = _LY.CT + (i // cols3) * (ch3 + gap3)
            color3 = accents[i % len(accents)]
            icon3  = card.get('icon', '') if isinstance(card, dict) else ''
            ttl3   = card.get('title', '') if isinstance(card, dict) else str(card)
            body3  = card.get('body', '') if isinstance(card, dict) else ''
            r_c3 = int(dk1[0:2], 16)
            g_c3 = int(dk1[2:4], 16)
            b_c3 = int(dk1[4:6], 16)
            dk1_l3 = (f'{min(255,int(r_c3+(255-r_c3)*0.18)):02X}'
                      f'{min(255,int(g_c3+(255-g_c3)*0.18)):02X}'
                      f'{min(255,int(b_c3+(255-b_c3)*0.18)):02X}')
            _h2_rounded_rect(slide, cx3, cy3, cw3, ch3, dk1_l3, _LY.R_SM)
            _h2_rect(slide, cx3, cy3, cw3, 0.055, color3)
            y3 = cy3 + 0.18
            if icon3:
                _h2_text(slide, icon3, left=cx3 + _LY.PAD, top=y3, width=0.40, height=0.36,
                         font=font, size_pt=16, color=color3, bold=False, align='left')
                y3 += 0.38
            _h2_text(slide, ttl3, left=cx3 + _LY.PAD, top=y3,
                     width=cw3 - _LY.PAD * 2, height=0.40,
                     font=font, size_pt=_LY.T_TITLE, color='FFFFFF', bold=True, align='left')
            y3 += 0.42
            if body3:
                _h2_text(slide, body3, left=cx3 + _LY.PAD, top=y3,
                         width=cw3 - _LY.PAD * 2, height=cy3 + ch3 - y3 - 0.10,
                         font=font, size_pt=_LY.T_SMALL, color='AAAAAA',
                         bold=False, align='left', line_spacing=1.2)
        if sb_cta:
            _h2_rounded_rect(slide, _LY.CL, _LY.CB - 0.64, _LY.CW, 0.54, accent1, _LY.R_SM)
            _h2_text(slide, sb_cta, left=_LY.CL, top=_LY.CB - 0.56,
                     width=_LY.CW, height=0.42,
                     font=font, size_pt=12, color='FFFFFF', bold=True, align='center')

    elif v == 4:
        # v4: message centré fort + icônes CTA en rangée bas
        msg4 = sb_quote or sb_title or ''
        cta4 = sb_cta or ''
        n4   = max(len(cards), 1)
        cols4 = min(n4, 4)
        gap4  = _LY.GAP_SM
        cw4   = (_LY.CW - gap4 * (cols4 - 1)) / cols4
        row_h4 = 1.50
        msg_bot4 = _LY.CB - row_h4 - 0.16

        if msg4:
            _h2_text(slide, msg4,
                     left=_LY.CL + 0.20, top=_LY.CT + 0.10,
                     width=_LY.CW - 0.40, height=msg_bot4 - _LY.CT - 0.20,
                     font=font, size_pt=20, color=dk1,
                     bold=False, italic=True, align='center', line_spacing=1.4)
            _h2_rect(slide, left=_LY.CL + _LY.CW * 0.35, top=msg_bot4 - 0.12,
                     width=_LY.CW * 0.30, height=0.04, color=accent1)

        c_top4 = _LY.CB - row_h4
        for i, card in enumerate(cards[:cols4]):
            cx4   = _LY.CL + i * (cw4 + gap4)
            color4 = accents[i % len(accents)]
            icon4  = card.get('icon', '') if isinstance(card, dict) else ''
            ttl4   = card.get('title', '') if isinstance(card, dict) else str(card)
            _h2_rounded_rect(slide, cx4, c_top4, cw4, row_h4, color4, _LY.R_SM)
            if icon4:
                _h2_text(slide, icon4, left=cx4, top=c_top4 + 0.14,
                         width=cw4, height=0.46,
                         font=font, size_pt=22, color='FFFFFF', bold=False, align='center')
            _h2_text(slide, ttl4, left=cx4 + _LY.PAD, top=c_top4 + row_h4 - 0.52,
                     width=cw4 - _LY.PAD * 2, height=0.44,
                     font=font, size_pt=_LY.T_TITLE, color='FFFFFF', bold=True, align='center')

        if cta4:
            _h2_text(slide, f'→  {cta4}',
                     left=_LY.CL, top=_LY.CT + 0.06,
                     width=_LY.CW, height=0.30,
                     font=font, size_pt=10, color=accent1,
                     bold=True, align='right')
...
def layout_agenda_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 2:
        ...
        dot_r   = 0.16
        _h2_rect(slide, left=line_x - 0.02, top=_LY.CT + step_h * 0.5,
                 width=0.04, height=(_LY.CB - _LY.CT) - step_h, color='DDDDDD')
        for i in range(n):
            number, label, sublabel = _item_data(items[i], i)
            color = accents[i % len(accents)]
            cy    = _LY.CT + (i + 0.5) * step_h
            # Dot
            _h2_rect(slide, left=line_x - dot_r, top=cy - dot_r,
                     width=dot_r * 2, height=dot_r * 2, color=color)
            _h2_text(slide, str(number),
                     left=line_x - dot_r, top=cy - dot_r,
                     width=dot_r * 2, height=dot_r * 2,
                     font=font, size_pt=9, color='FFFFFF', bold=True, align='center')
            # Label (right of line, always)
            tx = line_x + dot_r + 0.16
            tw = _LY.CR - tx - 0.10
            lbl_h = 0.32 if sublabel else 0.42
            _h2_text(slide, label, left=tx, top=cy - lbl_h / 2,
                     width=tw, height=lbl_h,
                     font=font, size_pt=14, color=dk1, bold=False, align='left')
            if sublabel:
                _h2_text(slide, sublabel, left=tx, top=cy + lbl_h / 2 - 0.08,
                         width=tw, height=0.26,
                         font=font, size_pt=_LY.T_SMALL, color='888888',
                         bold=False, align='left')
            # Accent tick on line
            _h2_rect(slide, left=line_x - 0.12, top=cy - 0.015,
                     width=0.24, height=0.03, color=color)

    elif v == 4:
        # Grand numéro filigrane: chiffre géant très pâle + label en dk1
        step_h = (_LY.CB - _LY.CT) / n
        for i in range(n):
            number, label, sublabel = _item_data(items[i], i)
            color  = accents[i % len(accents)]
            iy     = _LY.CT + i * step_h
            r_c, g_c, b_c = int(color[0:2],16), int(color[2:4],16), int(color[4:6],16)
            ghost  = (f'{min(255,int(r_c+(255-r_c)*0.92)):02X}'
                      f'{min(255,int(g_c+(255-g_c)*0.92)):02X}'
                      f'{min(255,int(b_c+(255-b_c)*0.92)):02X}')
            # Ghost number
            _h2_text(slide, f'{number:02d}' if isinstance(number, int) else str(number),
                     left=_LY.CL - 0.08, top=iy,
                     width=1.60, height=step_h,
                     font=font, size_pt=70, color=ghost, bold=True, align='left')
            # Label text
            tx = _LY.CL + 1.40
            tw = _LY.CW - 1.40
            lbl_h = 0.36 if sublabel else min(0.48, step_h - 0.10)
            _h2_text(slide, label,
                     left=tx, top=iy + (step_h - lbl_h) / 2 - (0.16 if sublabel else 0),
                     width=tw, height=lbl_h,
                     font=font, size_pt=16, color=dk1, bold=True, align='left')
            if sublabel:
                _h2_text(slide, sublabel,
                         left=tx, top=iy + (step_h + lbl_h) / 2 - 0.14,
                         width=tw, height=0.28,
                         font=font, size_pt=_LY.T_SMALL, color='888888',
                         bold=False, align='left')
            if i < n - 1:
                _h2_rect(slide, left=_LY.CL, top=iy + step_h - 0.006,
                         width=_LY.CW, height=0.006, color='EEEEEE')
...
def layout_matrix_v4(prs: Presentation, content: dict, tp: dict):
    """
    Matrice 2×2 — 3 variantes.
    v0 : quadrants neutres + fine bordure couleur en haut + axes labels.
    v1 : quadrants aux fonds colorés légers (accent teinté) + numéro grand en déco.
    v2 : lignes d'axes centrales + quadrants sans fond + label grand + body compact.
    content: {title, quadrants:[{label,body,items?}]×4, axes:{x,y}, footer}
    """
    slide  = _blank_v4(prs, tp)
    font   = tp.get('font', 'Calibri')
    theme  = tp.get('theme', {})
    ...
    if x_axis:
        _h2_text(slide, x_axis,
                 left=grid_x + grid_w / 4, top=_LY.CB - ax_lbl_h,
                 width=grid_w / 2, height=ax_lbl_h,
                 font=font, size_pt=10, color=dk1, bold=False, align='center')

    if v == 0:
        # Quadrants neutres + fine bordure couleur en haut
        for i, pos in enumerate(positions):
            ...
            bg = _cbg(tp, 0)
            _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, bg, _LY.R_SM)
            _h2_rect(slide, cx, cy, cell_w, 0.07, color)
            _h2_text(slide, label,
                     left=cx + _LY.PAD, top=cy + 0.13,
                     width=cell_w - _LY.PAD * 2, height=0.38,
                     font=font, size_pt=_LY.T_LABEL, color=dk1, bold=True, align='left')
        ...
        # Fond coloré léger par quadrant + grand numéro décoratif
        for i, pos in enumerate(positions):
            if i >= len(quadrants):
                break
            q     = quadrants[i] if isinstance(quadrants[i], dict) else {'label': str(quadrants[i])}
            ...
            # Tinted background from accent
            try:
                r = int(color[0:2], 16); g = int(color[2:4], 16); b = int(color[4:6], 16)
                tint = f'{min(255,int(r+(255-r)*0.82)):02X}{min(255,int(g+(255-g)*0.82)):02X}{min(255,int(b+(255-b)*0.82)):02X}'
            except Exception:
                tint = 'EEEEEE'
            _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, tint, _LY.R_SM)

            # Grand numéro décoratif en fond
            _h2_text(slide, str(i + 1),
                     left=cx + cell_w * 0.60, top=cy + cell_h * 0.05,
                     width=cell_w * 0.38, height=cell_h * 0.80,
                     font=font, size_pt=90, color=color, bold=True, align='right')

            # Bordure colorée à gauche
            _h2_rect(slide, cx, cy, 0.07, cell_h, color)

            _h2_text(slide, label,
                     left=cx + 0.14, top=cy + 0.10,
                     width=cell_w * 0.60, height=0.42,
                     font=font, size_pt=13, color=color, bold=True, align='left')
            txt = body or '\n'.join(f'• {it}' for it in items[:4])
            if txt:
                _h2_text(slide, txt,
                         left=cx + 0.14, top=cy + 0.55,
                         width=cell_w * 0.78, height=cell_h - 0.63,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.2)

    elif v == 2:
        # Lignes d'axes centrales + quadrants ouverts (sans fond lourd)
        mid_x = grid_x + (cell_w + gap / 2)
        mid_y = grid_y + (cell_h + gap / 2)
        _h2_rect(slide, grid_x, mid_y - 0.015, grid_w, 0.03, accent1)
        _h2_rect(slide, mid_x - 0.015, grid_y, 0.03, grid_h, accent1)
        ...
        for i, pos in enumerate(positions):
            ...
            # Petit cercle coloré coin haut-gauche de chaque quadrant
            _h2_circle(slide, cx=cx + 0.24, cy=cy + 0.24, r=0.18, color=color)
            _h2_text(slide, str(i + 1),
                     left=cx + 0.06, top=cy + 0.06,
                     width=0.36, height=0.36,
                     font=font, size_pt=11, color='FFFFFF', bold=True, align='center')

            _h2_text(slide, label,
                     left=cx + 0.52, top=cy + 0.08,
                     width=cell_w - 0.60, height=0.42,
                     font=font, size_pt=13, color=dk1, bold=True, align='left')
            txt = body or '\n'.join(f'• {it}' for it in items[:5])
            if txt:
                _h2_text(slide, txt,
                         left=cx + _LY.PAD, top=cy + 0.58,
                         width=cell_w - _LY.PAD * 2, height=cell_h - 0.66,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.2)

    elif v == 3:
        # v3: quadrants pleins couleur tintés + séparateurs accent + initiale grande
        for i, pos in enumerate(positions):
            if i >= len(quadrants):
                break
            q      = quadrants[i] if isinstance(quadrants[i], dict) else {'label': str(quadrants[i])}
            ...
            body   = q.get('body', '')
            items  = q.get('items', [])
            r_mx = int(color[0:2], 16)
            g_mx = int(color[2:4], 16)
            b_mx = int(color[4:6], 16)
            tint_mx = (f'{min(255,int(r_mx+(255-r_mx)*0.85)):02X}'
                       f'{min(255,int(g_mx+(255-g_mx)*0.85)):02X}'
                       f'{min(255,int(b_mx+(255-b_mx)*0.85)):02X}')
            _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, tint_mx, _LY.R_SM)
            _h2_rect(slide, cx, cy, cell_w, 0.065, color)
            # Large decorative number top-right
            _h2_text(slide, str(i + 1),
                     left=cx + cell_w * 0.62, top=cy + 0.02,
                     width=cell_w * 0.36, height=cell_h * 0.70,
                     font=font, size_pt=80, color=color, bold=True, align='right')
            _set_shape_alpha(slide.shapes[-1], 20)
            _h2_text(slide, label,
                     left=cx + _LY.PAD, top=cy + 0.12,
                     width=cell_w * 0.65, height=0.40,
                     font=font, size_pt=13, color=color, bold=True, align='left')
            txt = body or '\n'.join(f'• {it}' for it in items[:5])
            if txt:
                _h2_text(slide, txt,
                         left=cx + _LY.PAD, top=cy + 0.60,
                         width=cell_w - _LY.PAD * 2, height=cell_h - 0.68,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.2)

    elif v == 4:
        # v4: quadrants sans fond + grande icône/label + bullets compacts
        for i, pos in enumerate(positions):
            if i >= len(quadrants):
                break
            q      = quadrants[i] if isinstance(quadrants[i], dict) else {'label': str(quadrants[i])}
            ...
            items  = q.get('items', [])
            icon   = q.get('icon', '')
            _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, 'F8F9FA', _LY.R_SM)
            # Colored top band
            _h2_rounded_rect(slide, cx, cy, cell_w, _LY.HEAD_H, color, _LY.R_SM)
            _h2_rect(slide, cx, cy + _LY.R_SM, cell_w, _LY.HEAD_H - _LY.R_SM, color)
            icon_txt = icon or str(i + 1)
            _h2_text(slide, icon_txt,
                     left=cx + _LY.PAD, top=cy + 0.06,
                     width=0.48, height=_LY.HEAD_H - 0.08,
                     font=font, size_pt=18, color='FFFFFF', bold=True, align='left')
            _h2_text(slide, label,
                     left=cx + _LY.PAD + 0.52, top=cy + 0.08,
                     width=cell_w - _LY.PAD - 0.58, height=_LY.HEAD_H - 0.12,
                     font=font, size_pt=12, color='FFFFFF', bold=True, align='left')
            txt = body or '\n'.join(f'• {it}' for it in items[:5])
            if txt:
                _h2_text(slide, txt,
                         left=cx + _LY.PAD, top=cy + _LY.HEAD_H + 0.12,
                         width=cell_w - _LY.PAD * 2, height=cell_h - _LY.HEAD_H - 0.18,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.2)
...
def layout_swot_v4(prs: Presentation, content: dict, tp: dict):
    """
    Analyse SWOT — 3 variantes.
    v0 : 2×2 grille, header coloré + fond carte + bullets.
    v1 : 4 panneaux pleine couleur (fond accent), grande initiale décorative + items blancs.
    v2 : grille compacte avec icône emoji + titre en couleur + items, sans fond coloré lourd.
    ...
    positions = [
        (_LY.CL,               _LY.CT),
        (_LY.CL + cell_w + gap, _LY.CT),
        (_LY.CL,               _LY.CT + cell_h + gap),
        (_LY.CL + cell_w + gap, _LY.CT + cell_h + gap),
    ]

    if v == 0:
        # Grille classique : header coloré + fond neutre + bullets
        for (label, letter, key, hdr_color, icon), (cx, cy) in zip(quadrant_defs, positions):
            items = content.get(key, [])
            bg = _cbg(tp, 0)
            _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, bg, _LY.R_SM)
            _h2_rect(slide, cx, cy, cell_w, _LY.HEAD_H, hdr_color)
            _h2_text(slide, f'{icon}  {label}',
                     left=cx + _LY.PAD, top=cy + _LY.GAP_XS,
                     width=cell_w - _LY.PAD * 2, height=_LY.HEAD_H - 0.08,
                     font=font, size_pt=_LY.T_HEADER, color='FFFFFF', bold=True, align='left')
            txt = '\n'.join(f'• {it}' for it in items[:6])
            if txt:
                _h2_text(slide, txt,
                         left=cx + _LY.PAD, top=cy + _LY.HEAD_H + _LY.GAP_SM,
                         width=cell_w - _LY.PAD * 2, height=cell_h - _LY.HEAD_H - _LY.GAP_MD,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.22)

    elif v == 1:
        # 4 panneaux pleins couleurs — grande initiale décorative + items blancs
        for (label, letter, key, hdr_color, icon), (cx, cy) in zip(quadrant_defs, positions):
            items = content.get(key, [])
            _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, hdr_color, _LY.R_SM)
            # Grande initiale décorative en fond
            _h2_text(slide, letter,
                     left=cx + cell_w * 0.50, top=cy + cell_h * 0.05,
                     width=cell_w * 0.48, height=cell_h * 0.85,
                     font=font, size_pt=110, color='FFFFFF', bold=True, align='right')
            # Label en haut
            _h2_text(slide, label,
                     left=cx + _LY.PAD, top=cy + 0.10,
                     width=cell_w * 0.62, height=0.40,
                     font=font, size_pt=14, color='FFFFFF', bold=True, align='left')
            # Items bullets blancs
            txt = '\n'.join(f'• {it}' for it in items[:5])
            if txt:
                _h2_text(slide, txt,
                         left=cx + _LY.PAD, top=cy + 0.52,
                         width=cell_w - _LY.PAD * 2, height=cell_h - 0.60,
                         font=font, size_pt=10, color='FFFFFF',
                         bold=False, align='left', line_spacing=1.22)

    elif v == 2:
        # Grille épurée : bordure colorée gauche + icône + titre coloré + items dk1
        for (label, letter, key, hdr_color, icon), (cx, cy) in zip(quadrant_defs, positions):
            items = content.get(key, [])
            bg = _cbg(tp, 0)
            _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, bg, _LY.R_SM)
            # Bordure gauche colorée
            _h2_rect(slide, cx, cy, 0.07, cell_h, hdr_color)
            # Icône + titre
            _h2_text(slide, f'{icon}',
                     left=cx + 0.14, top=cy + 0.08,
                     width=0.52, height=0.44,
                     font=font, size_pt=22, color=hdr_color, bold=False, align='left')
            _h2_text(slide, label,
                     left=cx + 0.70, top=cy + 0.10,
                     width=cell_w - 0.78, height=0.40,
                     font=font, size_pt=13, color=hdr_color, bold=True, align='left')
            # Séparateur
            _h2_rect(slide, cx + 0.14, cy + 0.56, cell_w - 0.22, 0.03, hdr_color)
            # Items
            txt = '\n'.join(f'• {it}' for it in items[:6])
            if txt:
                _h2_text(slide, txt,
                         left=cx + 0.14, top=cy + 0.65,
                         width=cell_w - 0.22, height=cell_h - 0.72,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.22)

    elif v == 3:
        # v3: panneaux bi-ton — 2 quadrants haut plein couleur + 2 bas tintés clairs
        for idx, ((label, letter, key, hdr_color, icon), (cx, cy)) in enumerate(zip(quadrant_defs, positions)):
            items = content.get(key, [])
            if idx < 2:
                _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, hdr_color, _LY.R_SM)
                _h2_text(slide, letter,
                         left=cx + cell_w * 0.56, top=cy + cell_h * 0.04,
                         width=cell_w * 0.42, height=cell_h * 0.72,
                         font=font, size_pt=100, color='FFFFFF', bold=True, align='right')
                ...
                r_sw = int(hdr_color[0:2], 16)
                g_sw = int(hdr_color[2:4], 16)
                b_sw = int(hdr_color[4:6], 16)
                tint_sw = (f'{min(255,int(r_sw+(255-r_sw)*0.88)):02X}'
                           f'{min(255,int(g_sw+(255-g_sw)*0.88)):02X}'
                           f'{min(255,int(b_sw+(255-b_sw)*0.88)):02X}')
                _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, tint_sw, _LY.R_SM)
                _h2_rect(slide, cx, cy, cell_w, 0.07, hdr_color)
                _h2_text(slide, f'{icon}  {label}',
                         left=cx + _LY.PAD, top=cy + 0.13,
                         width=cell_w - _LY.PAD * 2, height=0.38,
                         font=font, size_pt=13, color=hdr_color, bold=True, align='left')
        ...
        # v4: minimaliste — cartes blanches + badge rond coloré + label + items épurés
        for (label, letter, key, hdr_color, icon), (cx, cy) in zip(quadrant_defs, positions):
            items = content.get(key, [])
            _h2_rounded_rect(slide, cx, cy, cell_w, cell_h, 'FFFFFF', _LY.R_SM)
            badge_r = 0.28
            _h2_rounded_rect(slide, cx + _LY.PAD, cy + (0.56 - badge_r * 2) / 2 + 0.04,
                             badge_r * 2, badge_r * 2, hdr_color, badge_r)
            _h2_text(slide, icon,
                     left=cx + _LY.PAD, top=cy + (0.56 - badge_r * 2) / 2 + 0.04,
                     width=badge_r * 2, height=badge_r * 2,
                     font=font, size_pt=14, color='FFFFFF', bold=False, align='center')
            _h2_text(slide, label,
                     left=cx + _LY.PAD + badge_r * 2 + 0.12, top=cy + 0.10,
                     width=cell_w - _LY.PAD - badge_r * 2 - 0.20, height=0.40,
                     font=font, size_pt=13, color=hdr_color, bold=True, align='left')
            _h2_rect(slide, cx + _LY.PAD, cy + 0.56, cell_w - _LY.PAD * 2, 0.025, 'E0E0E0')
            txt = '\n'.join(f'• {it}' for it in items[:5])
            if txt:
                _h2_text(slide, txt,
                         left=cx + _LY.PAD, top=cy + 0.62,
                         width=cell_w - _LY.PAD * 2, height=cell_h - 0.70,
                         font=font, size_pt=_LY.T_SMALL, color=dk1,
                         bold=False, align='left', line_spacing=1.22)
...
def layout_proscons_v4(prs: Presentation, content: dict, tp: dict):
    ...
    v0: en-tête coloré + rangées alternées avec bordure gauche
    v1: panneaux pleine couleur + grandes lettres décoratives + texte blanc
    v2: minimaliste — bande top + icône colorée + bullets épurés
    ...
    for col_idx, (items, color, bg, label, sym) in enumerate(col_defs):
        ...
        if v == 0:
            ...
            # Panneau pleine couleur + grande lettre décorative watermark + texte blanc
            r_c, g_c, b_c = int(color[0:2],16), int(color[2:4],16), int(color[4:6],16)
            dark = f'{max(0,int(r_c*0.72)):02X}{max(0,int(g_c*0.72)):02X}{max(0,int(b_c*0.72)):02X}'
            _h2_rect(slide, left=cx, top=_LY.CT, width=col_w, height=_LY.CB - _LY.CT, color=color)
            _h2_text(slide, sym,
                     left=cx + col_w - 1.40, top=_LY.CT - 0.10,
                     width=1.35, height=1.60,
                     font=font, size_pt=130, color=dark, bold=True, align='right')
            _h2_text(slide, label,
                     left=cx + _LY.PAD, top=_LY.CT + 0.22,
                     width=col_w - _LY.PAD * 2, height=0.44,
                     font=font, size_pt=_LY.T_TITLE + 2, color='FFFFFF',
                     bold=True, align='left')
            _h2_rect(slide, left=cx + _LY.PAD, top=_LY.CT + 0.68,
                     width=col_w * 0.20, height=0.04, color='FFFFFF')
            y = _LY.CT + 0.84
            n = min(len(items), 7)
            for j, item in enumerate(items[:n]):
                _h2_text(slide, f'  {item}',
                         left=cx + _LY.PAD, top=y,
                         width=col_w - _LY.PAD * 2, height=item_h - 0.04,
                         font=font, size_pt=11, color='FFFFFF',
                         bold=False, align='left', line_spacing=1.15)
                y += item_h
            ...
            for j, item in enumerate(items[:n]):
                ...
                _h2_text(slide, str(item),
                         ...
                         font=font, size_pt=11, color=dk1,
                         bold=False, align='left', line_spacing=1.1)
                _h2_rect(slide, left=cx + _LY.PAD, top=iy + item_h * 0.5 - 0.06,
                         width=0.12, height=0.12, color=color)
            ...
            for j, item in enumerate(items[:n3]):
                ...
                _h2_text(slide, str(item),
                         left=cx + 0.34, top=iy3 + 0.04,
                         width=col_w - 0.40, height=item_h - 0.08,
                         font=font, size_pt=11, color=dk1,
                         bold=False, align='left', line_spacing=1.1)

        elif v == 4:
            # v4: fond plein mais moins opaque + timeline verticale de points
            r_c4, g_c4, b_c4 = int(color[0:2],16), int(color[2:4],16), int(color[4:6],16)
            tint4 = (f'{min(255,int(r_c4+(255-r_c4)*0.90)):02X}'
                     f'{min(255,int(g_c4+(255-g_c4)*0.90)):02X}'
                     f'{min(255,int(b_c4+(255-b_c4)*0.90)):02X}')
            _h2_rect(slide, cx, _LY.CT, col_w, _LY.CB - _LY.CT, tint4)
            _h2_rect(slide, cx, _LY.CT, col_w, 0.065, color)
            _h2_text(slide, sym,
                     left=cx + _LY.PAD, top=_LY.CT + 0.14,
                     width=0.40, height=0.38,
                     font=font, size_pt=22, color=color, bold=True, align='left')
            lbl4 = label.replace('✓  ', '').replace('✗  ', '')
            _h2_text(slide, lbl4,
                     left=cx + _LY.PAD + 0.46, top=_LY.CT + 0.16,
                     width=col_w - _LY.PAD - 0.52, height=0.34,
                     font=font, size_pt=_LY.T_TITLE, color=dk1, bold=True, align='left')
            ...
            for j, item in enumerate(items[:n4]):
                ...
                _h2_text(slide, str(item),
                         left=cx + _LY.PAD + 0.32, top=iy4 + 0.04,
                         width=col_w - _LY.PAD - 0.38, height=item_h - 0.08,
                         font=font, size_pt=11, color=dk1,
                         bold=False, align='left', line_spacing=1.1)
...
def layout_table_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: en-tête couleur sombre + lignes avec première colonne sombre + alternance douce
        r_tv, g_tv, b_tv = int(accent1[0:2],16), int(accent1[2:4],16), int(accent1[4:6],16)
        drk = (f'{max(0,int(r_tv*0.68)):02X}'
               f'{max(0,int(g_tv*0.68)):02X}'
               f'{max(0,int(b_tv*0.68)):02X}')
        lgt = (f'{min(255,int(r_tv+(255-r_tv)*0.92)):02X}'
               f'{min(255,int(g_tv+(255-g_tv)*0.92)):02X}'
               f'{min(255,int(b_tv+(255-b_tv)*0.92)):02X}')
        for c, hdr in enumerate(headers[:n_cols]):
            _set_cell(0, c, hdr, drk, 'FFFFFF', bold=True, size_pt=12)
        for r, row in enumerate(rows[:n_rows - 1]):
            bg = lgt if r % 2 == 0 else 'FFFFFF'
            for c in range(n_cols):
                val = row[c] if c < len(row) else ''
                is_first = (c == 0)
                _set_cell(r + 1, c, val, bg,
        ...
        # v4: en-tête accent teinté très léger + alternance neutre + 1ère col accent
        r_tv4, g_tv4, b_tv4 = int(accent1[0:2],16), int(accent1[2:4],16), int(accent1[4:6],16)
        hdr4 = (f'{min(255,int(r_tv4+(255-r_tv4)*0.70)):02X}'
                f'{min(255,int(g_tv4+(255-g_tv4)*0.70)):02X}'
                f'{min(255,int(b_tv4+(255-b_tv4)*0.70)):02X}')
        alt4 = (f'{min(255,int(r_tv4+(255-r_tv4)*0.95)):02X}'
                f'{min(255,int(g_tv4+(255-g_tv4)*0.95)):02X}'
                f'{min(255,int(b_tv4+(255-b_tv4)*0.95)):02X}')
        for c, hdr in enumerate(headers[:n_cols]):
            _set_cell(0, c, hdr, hdr4, 'FFFFFF', bold=True, size_pt=12)
        for r, row in enumerate(rows[:n_rows - 1]):
            bg = alt4 if r % 2 == 0 else 'FFFFFF'
            for c in range(n_cols):
                val = row[c] if c < len(row) else ''
                is_first = (c == 0)
                _set_cell(r + 1, c, val, bg,
...
def layout_team_grid_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        col_w  = _LY.CW / n_cols
        zone_h = _LY.CB - _LY.CT
        r_est  = min(0.72, col_w * 0.30)
        _ct_h  = r_est + 0.28 + 2 * r_est + 0.20 + 0.36 + 0.38 + 0.26 + 0.14
        _card_h = max(_ct_h, zone_h * 0.36)
        _y_ct   = _LY.CT + (zone_h - _card_h) * 0.28 if _card_h < zone_h * 0.65 else _LY.CT
        for i, m in enumerate(members[:n_cols]):
            icon, name, role, dept, sv, sl, body = _mf(m)
            color  = accents[i % len(accents)]
            card_x = _LY.CL + i * col_w
            _h2_card_bg(slide, card_x + _LY.GAP_SM, _y_ct,
                        col_w - _LY.GAP_SM * 2, _card_h, tp, i)
            _h2_rect(slide, left=card_x + _LY.GAP_SM, top=_y_ct,
            ...
            if body:
                _h2_text(slide, body, card_x + _LY.PAD, y,
                         col_w - _LY.PAD * 2, _y_ct + _card_h - y - (0.68 if sv else 0.12),
                         font, _LY.T_SMALL, '666666', bold=False, align='center',
                         line_spacing=1.2)
            if sv:
                _h2_rect(slide, left=card_x + _LY.PAD, top=_y_ct + _card_h - 0.66,
                         width=col_w - _LY.PAD * 2, height=0.025, color='DDDDDD')
                _h2_text(slide, sv, card_x + _LY.PAD, _y_ct + _card_h - 0.62,
                         col_w - _LY.PAD * 2, 0.38,
                         font, 20, color, bold=True, align='center')
            if sl:
                _h2_text(slide, sl, card_x + _LY.PAD, _y_ct + _card_h - 0.24,
        ...
        # v3: Liste accordéon — cercle avatar + nom + rôle + stat
        row_gap = _LY.GAP_XS
        row_h   = (_LY.CB - _LY.CT - row_gap * (n - 1)) / max(n, 1)
        for i, m in enumerate(members[:n]):
            icon, name, role, dept, sv, sl, body = _mf(m)
            ...
            ry    = _LY.CT + i * (row_h + row_gap)
            _h2_rounded_rect(slide, _LY.CL, ry, _LY.CW, row_h, 'F4F7FA', _LY.R_SM)
            r  = min(0.30, row_h * 0.40)
            cx = _LY.CL + 0.12 + r
            cy = ry + row_h / 2
            _h2_circle(slide, cx, cy, r, color)
            _h2_text(slide, icon, cx - r, cy - r * 0.55, r * 2, r * 1.1,
                     font, int(r * 28), 'FFFFFF', bold=True, align='center')
            tx  = cx + r + 0.14
            tw  = _LY.CW * 0.58
            cy2 = ry + (row_h - (0.54 if role else 0.28)) / 2
            _h2_text(slide, name, tx, cy2, tw, 0.28,
                     font, _LY.T_LABEL, dk1, bold=True, align='left')
            if role:
                _h2_text(slide, role, tx, cy2 + 0.30, tw, 0.22,
                         font, _LY.T_SMALL, '777777', bold=False, align='left')
            if dept:
                _h2_text(slide, dept, tx, cy2 + (0.54 if role else 0.30), tw, 0.22,
                         font, 9, color, bold=True, align='left')
            if sv:
                sx = _LY.CL + _LY.CW * 0.68
                _h2_text(slide, sv, sx, ry + (row_h - 0.36) / 2 - (0.12 if sl else 0),
                         _LY.CR - sx - 0.08, 0.36,
                         font, 20, color, bold=True, align='center')
                if sl:
                    _h2_text(slide, sl, sx, ry + row_h / 2 + 0.10,
                             _LY.CR - sx - 0.08, 0.20,
                             font, 8, '999999', bold=False, align='center')

    elif v == 4:
        # v4: Cartes splitées — panneau coloré gauche (avatar) + fond clair droite (texte)
        n_cols  = 2 if n > 3 else min(n, 3)
        n_rows  = (n + n_cols - 1) // n_cols
        gap_x   = _LY.GAP_SM
        gap_y   = _LY.GAP_SM
        card_w  = (_LY.CW - gap_x * (n_cols - 1)) / n_cols
        ...
        for i, m in enumerate(members[:n]):
            ...
            col_i  = i % n_cols
            row_i  = i // n_cols
            cxc    = _LY.CL + col_i * (card_w + gap_x)
            cyc    = _LY.CT + row_i * (card_h + gap_y)
            _h2_rounded_rect(slide, cxc, cyc, card_w, card_h, 'F8F8F8', _LY.R_SM)
            _h2_rect(slide, cxc, cyc, panel_w, card_h, color)
            r  = min(0.44, panel_w * 0.44)
            ax = cxc + panel_w / 2
            ay = cyc + card_h / 2
            _h2_circle(slide, ax, ay, r, _darken(color, 0.75))
            _h2_text(slide, icon, ax - r, ay - r * 0.55, r * 2, r * 1.1,
                     font, int(r * 30), 'FFFFFF', bold=True, align='center')
            tx  = cxc + panel_w + 0.14
            tw  = card_w - panel_w - 0.22
            ty  = cyc + 0.14
            _h2_text(slide, name, tx, ty, tw, 0.32,
                     font, _LY.T_LABEL, dk1, bold=True, align='left')
            ty += 0.34
            if role:
                _h2_text(slide, role, tx, ty, tw, 0.24,
            ...
            if dept:
                ...
                ty += 0.24
            if body:
                _h2_text(slide, body, tx, ty, tw,
                         max(0.22, card_h - (ty - cyc) - (0.58 if sv else 0.12)),
                         font, _LY.T_SMALL, '555555', bold=False, align='left', line_spacing=1.2)
            if sv:
                sy = cyc + card_h - 0.56
                _h2_rect(slide, left=tx, top=sy, width=tw, height=0.025, color='EEEEEE')
                _h2_text(slide, sv, tx, sy + 0.04, tw, 0.36,
                         font, 18, color, bold=True, align='left')
                if sl:
                    _h2_text(slide, sl, tx, sy + 0.40, tw, 0.18,
                             font, 8, '999999', bold=False, align='left')
...
def layout_stat_banner_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: Hexagone accent en tête + valeur + label + séparateurs verticaux
        for i, st in enumerate(stats[:n]):
            val, label, sub, icon = _sf(st)
            color  = accents[i % len(accents)]
            cx     = _LY.CL + (i + 0.5) * cell_w
            hex_w, hex_h = 0.90, 0.80
            hex_x  = cx - hex_w / 2
            hex_y  = _LY.CT + zone_h * 0.06
            _h2_hexagon(slide, hex_x, hex_y, hex_w, hex_h, color)
            if icon:
                _h2_text(slide, icon, hex_x, hex_y, hex_w, hex_h,
                         font, 20, 'FFFFFF', bold=False, align='center')
            else:
                _h2_text(slide, str(i + 1), hex_x, hex_y, hex_w, hex_h,
                         font, 16, 'FFFFFF', bold=True, align='center')
            val_y = hex_y + hex_h + 0.16
            _h2_text(slide, val, _LY.CL + i * cell_w, val_y, cell_w, 0.80,
                     font, 40, color, bold=True, align='center')
            _h2_rect(slide, cx - 0.55, val_y + 0.82, 1.10, 0.025, color)
            _h2_text(slide, label, _LY.CL + i * cell_w, val_y + 0.90, cell_w, 0.32,
                     font, _LY.T_LABEL, dk1, bold=True, align='center')
            if sub:
                _h2_text(slide, sub, _LY.CL + i * cell_w, val_y + 1.24, cell_w, 0.26,
                         font, _LY.T_SMALL, '888888', bold=False, align='center')
            if i < n - 1:
                _h2_rect(slide, left=_LY.CL + (i + 1) * cell_w - 0.01,
                         top=_LY.CT + 0.30, width=0.02, height=zone_h - 0.60, color='DDDDDD')

    elif v == 4:
        # v4: Panneaux pleine couleur — fond coloré, valeur + label en blanc
        gap    = _LY.GAP_SM
        card_w = (_LY.CW - gap * (n - 1)) / max(n, 1)
        for i, st in enumerate(stats[:n]):
            val, label, sub, icon = _sf(st)
            color  = accents[i % len(accents)]
            cx2    = _LY.CL + i * (card_w + gap)
            _h2_rounded_rect(slide, cx2, _LY.CT, card_w, zone_h, color, _LY.R_SM)
            cy_mid = _LY.CT + zone_h / 2
            y      = cy_mid - (1.0 if icon else 0.70)
            if icon:
                _h2_text(slide, icon, cx2, y, card_w, 0.46,
                         font, 28, 'FFFFFF', bold=True, align='center')
                y += 0.50
            _h2_text(slide, val, cx2, y, card_w, 0.80,
                     font, 44, 'FFFFFF', bold=True, align='center')
            y += 0.82
            _h2_rect(slide, cx2 + card_w * 0.25, y, card_w * 0.50, 0.025, 'FFFFFF')
            y += 0.06
            _h2_text(slide, label, cx2, y, card_w, 0.32,
                     font, _LY.T_LABEL, 'FFFFFF', bold=True, align='center')
            if sub:
                _h2_text(slide, sub, cx2, y + 0.34, card_w, 0.26,
                         font, _LY.T_SMALL, 'FFFFFF', bold=False, align='center')
...
def layout_icon_row_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        _cw_est  = col_w - _LY.GAP_SM * 2
        _r_est   = min(0.50, _cw_est * 0.22)
        _ct_h    = 0.22 + 2 * _r_est + 0.22 + 0.40 + 0.44 + 0.30 + 0.14
        _card_h  = max(_ct_h, zone_h * 0.36)
        _y_ct    = _LY.CT + (zone_h - _card_h) * 0.28 if _card_h < zone_h * 0.65 else _LY.CT
        for i, it in enumerate(items[:n]):
            icon, title, body, bullets = _if(it)
            ...
            _h2_rounded_rect(slide, left=cx_card, top=_y_ct,
                             width=cw, height=_card_h, color=_cbg(tp, i), radius=_LY.R_SM)
            _h2_rect(slide, left=cx_card, top=_y_ct, width=cw, height=0.05, color=color)
            cx = cx_card + cw / 2
            r  = min(0.50, cw * 0.22)
            cy = _y_ct + r + 0.22
            _h2_circle(slide, cx, cy, r, color)
            ...
            if body_src:
                _h2_text(slide, body_src, cx_card + _LY.PAD, y,
                         cw - _LY.PAD * 2, _y_ct + _card_h - y - 0.14,
        ...
        # v3: Bandes accordéon — cercle icône + titre + corps/bullets par ligne
        row_gap = _LY.GAP_XS
        row_h   = (_LY.CB - _LY.CT - row_gap * (n - 1)) / max(n, 1)
        for i, it in enumerate(items[:n]):
            icon, title, body, bullets = _if(it)
            ...
            if body_src:
                ty = ry + row_h * 0.12
                _h2_text(slide, title, tx, ty, tw, min(0.34, row_h * 0.44),
                         font, _LY.T_TITLE, dk1, bold=True, align='left')
                _h2_text(slide, body_src, tx, ty + 0.36, tw, max(0.22, row_h * 0.88 - 0.46),
                         font, _LY.T_SMALL, '555555', bold=False, align='left', line_spacing=1.2)
            else:
                _h2_text(slide, title, tx, ry + (row_h - 0.32) / 2, tw, 0.32,
                         font, _LY.T_TITLE, dk1, bold=True, align='left')

    elif v == 4:
        # v4: Colonnes pleine couleur — fond coloré, icône + titre + corps en blanc
        for i, it in enumerate(items[:n]):
            icon, title, body, bullets = _if(it)
            color   = accents[i % len(accents)]
            card_x  = _LY.CL + i * col_w + _LY.GAP_XS
            card_w2 = col_w - _LY.GAP_XS * 2
            _h2_rounded_rect(slide, card_x, _LY.CT, card_w2, zone_h, color, _LY.R_SM)
            r  = min(0.50, card_w2 * 0.22)
            cx = card_x + card_w2 / 2
            cy = _LY.CT + r + 0.24
            _h2_circle(slide, cx, cy, r, _darken(color, 0.75))
            _h2_text(slide, icon, cx - r, cy - r * 0.55, r * 2, r * 1.1,
                     font, int(r * 44), 'FFFFFF', bold=True, align='center')
            y = cy + r + 0.22
            _h2_text(slide, title, card_x + _LY.PAD, y, card_w2 - _LY.PAD * 2, 0.40,
                     font, _LY.T_TITLE, 'FFFFFF', bold=True, align='center')
            y += 0.44
            body_src = body or ('\n'.join(f'• {x}' for x in bullets[:3]))
            if body_src:
                _h2_text(slide, body_src, card_x + _LY.PAD, y,
                         card_w2 - _LY.PAD * 2, _LY.CB - y - 0.14,
                         font, _LY.T_SMALL, 'FFFFFF', bold=False, align='center',
                         line_spacing=1.2)
...
def layout_section_break_v4(prs: Presentation, content: dict, tp: dict):
    ...
    v3 : bande horizontale accent centrée pleine largeur + titre blanc + numéro fantôme.
    v4 : fond dk1 + trois cercles déco droite + titre blanc gauche + badge numéro.
    ...
    if v == 0:
        ...
        # v3 : bande horizontale accent centrée pleine largeur + titre blanc + numéro fantôme
        r2, g2, b2 = int(dk1[:2], 16), int(dk1[2:4], 16), int(dk1[4:], 16)
        ghost = (f'{min(255,int(r2+(255-r2)*0.88)):02X}'
                 f'{min(255,int(g2+(255-g2)*0.88)):02X}'
                 f'{min(255,int(b2+(255-b2)*0.88)):02X}')
        if number:
            _h2_text(slide, number, -0.20, H * 0.04, 2.20, 1.80,
                     font, 160, ghost, bold=True, align='left')
        _h2_rounded_rect(slide, 0, H * 0.34, W, H * 0.32, accent1, _LY.R_MD)
        _h2_text(slide, title, W * 0.06, H * 0.37, W * 0.88, 0.76,
                 font, 32, 'FFFFFF', bold=True, align='center')
        if subtitle:
            _h2_text(slide, subtitle, W * 0.15, H * 0.57, W * 0.70, 0.36,
                     font, 13, 'EEEEEE', bold=False, align='center')

    elif v == 4:
        # v4 : fond dk1 + trois cercles déco droite + titre blanc gauche + badge numéro
        _h2_rect(slide, left=0, top=0, width=W, height=H, color=dk1)
        r2, g2, b2 = int(dk1[:2], 16), int(dk1[2:4], 16), int(dk1[4:], 16)
        dk1_light = (f'{min(255,int(r2+(255-r2)*0.15)):02X}'
                     f'{min(255,int(g2+(255-g2)*0.15)):02X}'
                     f'{min(255,int(b2+(255-b2)*0.15)):02X}')
        _h2_circle(slide, W * 0.82, H * 0.50, 2.80, dk1_light)
        _h2_circle(slide, W * 0.82, H * 0.50, 2.00, _darken(accent1, 0.50))
        _h2_circle(slide, W * 0.82, H * 0.50, 1.30, dk1)
        if number:
            _h2_rounded_rect(slide, W * 0.06, H * 0.20, 0.60, 0.44, accent1, _LY.R_SM)
            _h2_text(slide, str(number), W * 0.06, H * 0.20, 0.60, 0.44,
                     font, 16, 'FFFFFF', bold=True, align='center')
        _h2_rect(slide, left=W * 0.06, top=H * 0.72, width=1.60, height=0.04, color=accent1)
        _h2_text(slide, title, W * 0.06, H * 0.34, W * 0.55, 0.72,
                 font, 34, 'FFFFFF', bold=True, align='left')
        if subtitle:
            _h2_text(slide, subtitle, W * 0.06, H * 0.60, W * 0.55, 0.36,
                     font, 13, 'CCCCCC', bold=False, align='left')

    if footer:
        fg = 'AAAAAA' if v in (2, 4) else '999999'
...
def layout_photo_text_v4(prs: Presentation, content: dict, tp: dict):
    ...
    v3 : photo bande pleine largeur en haut + items en colonnes bas.
    v4 : photo gauche étroite portrait + items droite en grille 2 colonnes.
    ...
    if v == 0:
        ...
        # v3 : photo bande pleine largeur haut + items en colonnes bas
        ph = zone_h * 0.40
        _photo_zone(_LY.CL, _LY.CT, _LY.CW, ph, accent1)
        n = min(len(items), 4)
        if n > 0:
            item_y = _LY.CT + ph + _LY.GAP_SM
            item_h = _LY.CB - item_y
            col_w2 = (_LY.CW - _LY.GAP_SM * (n - 1)) / n
            for i, it in enumerate(items[:n]):
                cx2 = _LY.CL + i * (col_w2 + _LY.GAP_SM)
                _item_row(it, cx2, item_y, col_w2, item_h, accents[i % len(accents)])

    elif v == 4:
        # v4 : photo gauche étroite portrait + items droite grille 2 colonnes
        pw = _LY.CW * 0.34
        _photo_zone(_LY.CL, _LY.CT, pw, zone_h, accent1)
        tx = _LY.CL + pw + _LY.GAP_SM
        tw = _LY.CW - pw - _LY.GAP_SM
        n = min(len(items), 4)
        if n > 0:
            n_cols = 2
            col_w2 = (tw - _LY.GAP_SM) / 2
            n_rows = (n + 1) // 2
            row_h2 = zone_h / n_rows
            for i, it in enumerate(items[:n]):
                col_i = i % n_cols
                row_i = i // n_cols
                ix2 = tx + col_i * (col_w2 + _LY.GAP_SM)
                iy2 = _LY.CT + row_i * row_h2
                _item_row(it, ix2, iy2, col_w2, row_h2, accents[i % len(accents)])
...
def layout_numbered_features_v4(prs: Presentation, content: dict, tp: dict):
    ...
    v3 : accordion rows — panneau gauche coloré + grand numéro + titre/corps droite.
    v4 : grille 2 colonnes — bande accent haut + badge numéro + titre + corps.
    ...
    if v == 0:
        ...
        n_cols  = n
        col_w   = (_LY.CW - _LY.GAP_LG * (n_cols - 1)) / n_cols
        _has_sv = any(it.get('stat_value', '') for it in items[:n] if isinstance(it, dict))
        _max_bl = max((len(it.get('body', '') if isinstance(it, dict) else '') for it in items[:n]), default=0)
        _bh_est = min(0.28 * max(1, _max_bl // 50 + 1), 1.0) if _max_bl else 0
        _ct_h   = 0.96 + _bh_est + (0.70 if _has_sv else 0.14)
        _card_h = max(_ct_h, zone_h * 0.36)
        _y_ct   = _LY.CT + (zone_h - _card_h) * 0.28 if _card_h < zone_h * 0.65 else _LY.CT
        for i, it in enumerate(items[:n]):
            ...
            # Fond carte
            _h2_card_bg(slide, cx, _y_ct, col_w, _card_h, tp, i)
            # Grand numéro décoratif en fond (couleur très légère)
            _h2_text(slide, num, cx - 0.06, _y_ct + 0.06, col_w + 0.12, 0.96,
                     font, 72, _cbg(tp, i + 1), bold=True, align='right')
            # Barre accent
            _h2_rect(slide, left=cx + _LY.PAD, top=_y_ct + 0.20,
                     width=0.80, height=0.04, color=color)
            # Titre
            _h2_text(slide, title, cx + _LY.PAD, _y_ct + 0.30,
                     col_w - _LY.PAD * 2, 0.52,
                     font, _LY.T_HEADER, dk1, bold=True, align='left')
            # Séparateur
            _h2_rect(slide, left=cx + _LY.PAD, top=_y_ct + 0.88,
                     width=col_w - _LY.PAD * 2, height=0.02, color='DDDDDD')
            # Corps
            if body:
                body_h = _card_h - 0.96 - (0.70 if sv else 0.14)
                _h2_text(slide, body, cx + _LY.PAD, _y_ct + 0.96,
                         col_w - _LY.PAD * 2, max(0.20, body_h),
                         font, _LY.T_SMALL, '555555', bold=False, align='left',
                         line_spacing=1.3)
            if sv:
                _h2_text(slide, sv, cx + _LY.PAD, _y_ct + _card_h - 0.62,
                         col_w - _LY.PAD * 2, 0.38,
                         font, 22, color, bold=True, align='left')
            if sl:
                _h2_text(slide, sl, cx + _LY.PAD, _y_ct + _card_h - 0.24,
                         col_w - _LY.PAD * 2, 0.20,
        ...
        # v3 : accordion rows — panneau gauche coloré + grand numéro + titre/corps droite
        n_rows  = min(n, 4)
        row_h   = (zone_h - _LY.GAP_XS * (n_rows - 1)) / n_rows
        panel_w = 1.80
        for i, it in enumerate(items[:n_rows]):
            num, title, body, sv, sl = _ff(it, i)
            color  = accents[i % len(accents)]
            ry     = _LY.CT + i * (row_h + _LY.GAP_XS)
            _h2_rounded_rect(slide, _LY.CL, ry, _LY.CW, row_h, 'F4F7FA', _LY.R_SM)
            _h2_rounded_rect(slide, _LY.CL, ry, panel_w, row_h, color, _LY.R_SM)
            _h2_rect(slide, left=_LY.CL + panel_w - 0.10, top=ry,
                     width=0.10, height=row_h, color=color)
            _h2_text(slide, num, _LY.CL, ry, panel_w, row_h,
        ...
        for i, it in enumerate(items[:n]):
            ...
            cx     = _LY.CL + col_i * (card_w + _LY.GAP_LG)
            cy     = _LY.CT + row_i * (card_h + _LY.GAP_SM)
            _h2_rounded_rect(slide, cx, cy, card_w, card_h, 'FFFFFF', _LY.R_SM)
            _h2_rounded_rect(slide, cx, cy, card_w, 0.20, color, _LY.R_SM)
            _h2_rect(slide, left=cx, top=cy + 0.10, width=card_w, height=0.10, color=color)
            _h2_rounded_rect(slide, cx + _LY.PAD, cy + 0.30, 0.46, 0.30, color, _LY.R_SM)
            _h2_text(slide, num, cx + _LY.PAD, cy + 0.30, 0.46, 0.30,
                     font, 13, 'FFFFFF', bold=True, align='center')
            tx  = cx + _LY.PAD + 0.56
            tw  = card_w - _LY.PAD - 0.66
            _h2_text(slide, title, tx, cy + 0.28, tw, 0.36,
                     font, _LY.T_HEADER, dk1, bold=True, align='left')
            if body:
                _h2_text(slide, body, cx + _LY.PAD, cy + 0.68, card_w - _LY.PAD * 2,
                         card_h - 0.82,
                         font, _LY.T_SMALL, '666666', bold=False, align='left',
                         line_spacing=1.2)
            if sv:
                _h2_text(slide, sv, cx + _LY.PAD, cy + card_h - 0.52,
                         card_w - _LY.PAD * 2, 0.36,
                         font, 18, color, bold=True, align='left')
...
def layout_side_panel_v4(prs: Presentation, content: dict, tp: dict):
    ...
    v3 : bandeau accent en haut + items en grille 2-3 colonnes sans sidebar.
    v4 : sidebar étroit dk1 gauche (1.30") + items en 3 colonnes compactes.
    ...
    if v == 0:
        ...
        # v3 : bandeau accent haut + items en grille 2-3 colonnes sans sidebar
        band_h = 0.96
        _h2_rect(slide, left=0, top=0, width=W, height=band_h, color=accent1)
        _h2_text(slide, main_title, _LY.CL, 0.20, _LY.CW, 0.58,
                 font, 22, 'FFFFFF', bold=True, align='left')
        n = min(len(items), 6)
        if n:
            n_cols = 3 if n > 3 else 2
            col_w3 = (_LY.CW - _LY.GAP_MD * (n_cols - 1)) / n_cols
            n_rows = (n + n_cols - 1) // n_cols
            row_h3 = (H - band_h - _LY.GAP_SM - (0.32 if footer else 0.10)) / n_rows
            for i, it in enumerate(items[:n]):
                col_i = i % n_cols
                row_i = i // n_cols
                icon  = it.get('icon', '') if isinstance(it, dict) else ''
                ...
                body  = it.get('body', '') if isinstance(it, dict) else ''
                color = accents[i % len(accents)]
                ix3   = _LY.CL + col_i * (col_w3 + _LY.GAP_MD)
                iy3   = band_h + _LY.GAP_SM + row_i * row_h3
                _h2_rounded_rect(slide, left=ix3, top=iy3 + 0.04,
                                 width=col_w3, height=row_h3 - 0.08,
                                 color=_cbg(tp, i), radius=_LY.R_SM)
                _h2_rect(slide, left=ix3, top=iy3 + 0.04,
                         width=0.04, height=row_h3 - 0.08, color=color)
                tx3 = ix3 + 0.14
                if icon:
                    _h2_icon_circle(slide, tx3 + 0.22, iy3 + row_h3 / 2,
                                    0.20, icon, font, color, 'FFFFFF', 10)
                    tx3 += 0.54
                tw3 = ix3 + col_w3 - tx3 - 0.10
                _h2_text(slide, title, tx3, iy3 + 0.10, tw3, 0.30,
                         font, _LY.T_SMALL + 1, dk1, bold=True, align='left')
                if body:
                    _h2_text(slide, body, tx3, iy3 + 0.42, tw3, row_h3 - 0.50,
                             font, _LY.T_SMALL, '666666', bold=False, align='left',
                             line_spacing=1.1)
        ...
        # v4 : sidebar étroit dk1 gauche (1.30") + items en 3 colonnes compactes
        sw = 1.30
        _h2_rect(slide, left=0, top=0, width=sw, height=H, color=dk1)
        _h2_rect(slide, left=0, top=0, width=sw, height=0.24, color=accent1)
        _vertical_text(0, 0.28, sw, H - 0.56, panel_title, 'FFFFFF', 14)
        cx4 = sw + 0.44
        cw4 = W - cx4 - 0.36
        _h2_text(slide, main_title, cx4, 0.16, cw4, 0.52,
                 font, 20, dk1, bold=True, align='left')
        _h2_rect(slide, left=cx4, top=0.72, width=1.40, height=0.03, color=accent1)
        n = min(len(items), 6)
        if n:
            n_cols = 3 if n > 3 else 2
            col_w4 = (cw4 - _LY.GAP_SM * (n_cols - 1)) / n_cols
            n_rows = (n + n_cols - 1) // n_cols
            row_h4 = (H - 0.88 - (0.30 if footer else 0.10)) / n_rows
            for i, it in enumerate(items[:n]):
                col_i = i % n_cols
                row_i = i // n_cols
                icon  = it.get('icon', '') if isinstance(it, dict) else ''
                ...
                body  = it.get('body', '') if isinstance(it, dict) else ''
                color = accents[i % len(accents)]
                ix4   = cx4 + col_i * (col_w4 + _LY.GAP_SM)
                iy4   = 0.88 + row_i * row_h4
                _h2_rounded_rect(slide, left=ix4, top=iy4 + 0.04,
                                 width=col_w4, height=row_h4 - 0.08,
                                 color=_cbg(tp, i), radius=_LY.R_SM)
                _h2_rect(slide, left=ix4, top=iy4 + 0.04,
                         width=0.04, height=row_h4 - 0.08, color=color)
                tx4 = ix4 + 0.12
                if icon:
                    _h2_icon_circle(slide, tx4 + 0.20, iy4 + row_h4 / 2,
                                    0.18, icon, font, color, 'FFFFFF', 9)
                    tx4 += 0.50
                tw4 = ix4 + col_w4 - tx4 - 0.08
                _h2_text(slide, title, tx4, iy4 + 0.08, tw4, 0.28,
                         font, _LY.T_SMALL + 1, dk1, bold=True, align='left')
                if body:
                    _h2_text(slide, body, tx4, iy4 + 0.38, tw4, row_h4 - 0.46,
                             font, _LY.T_SMALL, '666666', bold=False, align='left',
                             line_spacing=1.1)

    if footer:
        fx = sw + 0.55 if v == 4 else (panel_w + 0.55 if v < 3 else _LY.CL)
        fw = W - fx - (0.10 if v >= 3 else 1.0)
        _h2_text(slide, footer, fx, H - 0.36, fw, 0.28,
...
def layout_circle_stats_v4(prs: Presentation, content: dict, tp: dict):
    ...
    v3 : rangées horizontales — zone colorée gauche avec valeur + label + barre à droite.
    v4 : cartes fond coloré plein + valeur géante blanche + trait + label + desc.
    ...
    for i, c in enumerate(circles[:n]):
        ...
        if v == 0:
            ...
            # v3 : rangées horizontales — zone colorée gauche + label + barre droite
            row_h = zone_h / n
            ry    = _LY.CT + i * row_h
            lh    = row_h - _LY.GAP_XS * 2
            _h2_rounded_rect(slide, _LY.CL, ry + _LY.GAP_XS, _LY.CW, lh, 'F4F7FA', _LY.R_SM)
            val_w = 1.70
            _h2_rounded_rect(slide, _LY.CL, ry + _LY.GAP_XS, val_w, lh, color, _LY.R_SM)
            _h2_rect(slide, left=_LY.CL + val_w - 0.08, top=ry + _LY.GAP_XS,
                     width=0.08, height=lh, color=color)
            _h2_text(slide, val, _LY.CL, ry + _LY.GAP_XS, val_w, lh,
                     font, 36, 'FFFFFF', bold=True, align='center')
            lx  = _LY.CL + val_w + 0.22
            lw  = _LY.CR - lx - 0.10
            cy3 = ry + row_h / 2
            _h2_text(slide, label, lx, cy3 - 0.22, lw * 0.55, 0.30,
                     font, _LY.T_LABEL, dk1, bold=True, align='left')
            try:
                pct = float(str(val).replace('%', '').strip()) / 100
            except Exception:
                pct = 0.5
            bar_x = lx + lw * 0.57
            bar_w = lw - lw * 0.57 - 0.10
            _h2_rect(slide, left=bar_x, top=cy3 - 0.06,
                     width=bar_w, height=0.12, color='DDDDDD')
            _h2_rect(slide, left=bar_x, top=cy3 - 0.06,
                     width=max(0.04, bar_w * pct), height=0.12, color=color)
            if desc:
                _h2_text(slide, desc, lx, cy3 + 0.14, lw, row_h / 2 - 0.22,
                         font, _LY.T_SMALL, '666666', bold=False, align='left',
                         line_spacing=1.2)

        elif v == 4:
            # v4 : cartes fond coloré plein + valeur géante blanche + trait + label
            card_w = col_w - _LY.GAP_XS * 2
            card_x = _LY.CL + i * col_w + _LY.GAP_XS
            card_cx = card_x + card_w / 2
            _h2_rounded_rect(slide, card_x, _LY.CT, card_w, zone_h, color, _LY.R_MD)
            _h2_text(slide, val, card_x + 0.10, _LY.CT + zone_h * 0.12,
                     card_w - 0.20, zone_h * 0.42,
                     font, _LY.T_KPI, 'FFFFFF', bold=True, align='center')
            _h2_rect(slide, left=card_cx - 0.80, top=_LY.CT + zone_h * 0.56,
                     width=1.60, height=0.04, color='FFFFFF')
            _h2_text(slide, label, card_x + 0.10, _LY.CT + zone_h * 0.60,
                     card_w - 0.20, 0.38,
                     font, _LY.T_LABEL, 'FFFFFF', bold=True, align='center')
            body_src = desc or ('\n'.join(f'• {b}' for b in bullets[:3]))
            if body_src:
                _h2_text(slide, body_src, card_x + _LY.PAD,
                         _LY.CT + zone_h * 0.72, card_w - _LY.PAD * 2,
                         zone_h * 0.24,
                         font, _LY.T_SMALL, 'EEEEEE', bold=False, align='center',
                         line_spacing=1.2)
...
def layout_mission_vision_v4(prs: Presentation, content: dict, tp: dict):
    ...
    v3 : panneaux empilés (rangées horizontales) — rangée colorée + rangée neutre.
    v4 : panneaux côte-à-côte — bande colorée haute (icon+titre) + corps neutre bas.
    ...
    for i, panel in enumerate(panels[:n]):
        ...
        if v == 3:
            # v3 : panneaux empilés (rangées pleine largeur)
            ph    = H / 2
            py    = i * ph
            color3 = accents[i % len(accents)]
            if i % 2 == 0:
                bg3, fg3 = color3, 'FFFFFF'
            else:
                bg3, fg3 = _cbg(tp, 0), dk1
            _h2_rect(slide, left=0, top=py, width=W, height=ph, color=bg3)
            if i > 0:
                _h2_rect(slide, left=0, top=py, width=W, height=0.02, color='DDDDDD')
            cx3 = W * 0.12
            cy3 = py + ph / 2
            if icon:
                r3 = 0.44
                circle_c = _darken(color3, 0.75) if fg3 == 'FFFFFF' else accent1
                _h2_circle(slide, cx3, cy3, r3, circle_c)
                _h2_text(slide, icon, cx3 - r3, cy3 - r3 * 0.55,
                         r3 * 2, r3 * 1.1, font, int(r3 * 40), fg3, bold=True, align='center')
                tx3 = cx3 + r3 + 0.24
            else:
                tx3 = cx3
            tw3 = W - tx3 - 0.40
            ty3 = py + ph * 0.22
            if label:
                _h2_text(slide, label, tx3, ty3, tw3, 0.26,
                         font, 9, fg3, bold=True, align='left')
                ty3 += 0.28
            _h2_rect(slide, left=tx3, top=ty3, width=1.40, height=0.03,
                     color='FFFFFF' if fg3 == 'FFFFFF' else accent1)
            ty3 += 0.10
            _h2_text(slide, title, tx3, ty3, tw3, 0.48,
                     font, 20, fg3, bold=True, align='left')
            ty3 += 0.52
            if body_src:
                _h2_text(slide, body_src, tx3, ty3, tw3, ph - ty3 + py - 0.16,
                         font, _LY.T_SMALL, fg3, bold=False, align='left', line_spacing=1.3)
            continue

        if v == 4:
            # v4 : panneaux côte-à-côte — bande colorée haute + corps neutre bas
            px4    = i * pw
            color4 = accents[i % len(accents)]
            top_h  = H * 0.44
            _h2_rect(slide, left=px4, top=0, width=pw, height=top_h, color=color4)
            _h2_rect(slide, left=px4, top=top_h, width=pw, height=H - top_h, color='F8F8F8')
            cy4 = 0
            if icon:
                r4 = 0.44
                ic_cx = px4 + pw / 2
                ic_cy = top_h * 0.28
                _h2_circle(slide, ic_cx, ic_cy, r4, _darken(color4, 0.75))
                _h2_text(slide, icon, ic_cx - r4, ic_cy - r4 * 0.55,
                         r4 * 2, r4 * 1.1, font, int(r4 * 40), 'FFFFFF', bold=True, align='center')
                cy4 = ic_cy + r4 + 0.14
            else:
                cy4 = top_h * 0.18
            if label:
                _h2_text(slide, label, px4 + 0.30, cy4, pw - 0.60, 0.26,
                         font, 9, 'EEEEEE', bold=True, align='center')
                cy4 += 0.28
            _h2_rect(slide, left=px4 + pw * 0.25, top=cy4,
                     width=pw * 0.50, height=0.03, color='FFFFFF')
            cy4 += 0.10
            _h2_text(slide, title, px4 + 0.30, cy4, pw - 0.60, 0.50,
                     font, 18, 'FFFFFF', bold=True, align='center')
            if body_src:
                body_h4 = H - top_h - (0.70 if footer else 0.20) - 0.26
                _h2_text(slide, body_src, px4 + 0.30, top_h + 0.22, pw - 0.60, body_h4,
                         font, _LY.T_SMALL, dk1, bold=False, align='center', line_spacing=1.3)
            continue
    ...
    # Séparateur vertical entre panneaux (v0/v1/v2 uniquement)
    if n == 2 and v < 3:
...
def layout_photo_grid_v4(prs: Presentation, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3 : grille 2×2 — 4 photos carrées avec légendes
        n4  = min(len(photos), 4)
        pw3 = (_LY.CW - _LY.GAP_SM) / 2
        ph3 = (zone_h - _LY.GAP_SM) / 2
        coords3 = [
            (_LY.CL,               _LY.CT),
            (_LY.CL + pw3 + _LY.GAP_SM, _LY.CT),
            (_LY.CL,               _LY.CT + ph3 + _LY.GAP_SM),
            (_LY.CL + pw3 + _LY.GAP_SM, _LY.CT + ph3 + _LY.GAP_SM),
        ]
        for i in range(n4):
            _photo_box(coords3[i][0], coords3[i][1], pw3, ph3,
                       photos[i], accents[i % len(accents)])

    elif v == 4:
        # v4 : grande photo pleine hauteur gauche + 2 photos milieu empilées
        n4b   = min(len(photos), 3)
        big_w = _LY.CW * 0.45
        mid_w = _LY.CW - big_w - _LY.GAP_SM
        sm_h  = (zone_h - _LY.GAP_SM) / 2
        sm_x  = _LY.CL + big_w + _LY.GAP_SM
        if n4b >= 1:
            _photo_box(_LY.CL, _LY.CT, big_w, zone_h, photos[0],
                       accents[0 % len(accents)])
        if n4b >= 2:
            _photo_box(sm_x, _LY.CT, mid_w, sm_h, photos[1],
                       accents[1 % len(accents)])
        if n4b >= 3:
            _photo_box(sm_x, _LY.CT + sm_h + _LY.GAP_SM, mid_w, sm_h, photos[2],
                       accents[2 % len(accents)])
...
def layout_pricing_table_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3 : bandes horizontales — une rangée par tier
        zone_h = _LY.CB - _LY.CT
        row_h  = (zone_h - _LY.GAP_XS * (n - 1)) / n
        name_w = 2.60
        for i, tier in enumerate(tiers):
            ry    = _LY.CT + i * (row_h + _LY.GAP_XS)
            color = accents[i % len(accents)]
            _h2_rounded_rect(slide, _LY.CL, ry, _LY.CW, row_h, 'F8F8F8', _LY.R_SM)
            _h2_rounded_rect(slide, _LY.CL, ry, name_w, row_h, color, _LY.R_SM)
            _h2_rect(slide, _LY.CL + name_w - 0.08, ry, 0.08, row_h, color)
            _h2_text(slide, tier.get('name', f'Plan {i+1}').upper(),
                     _LY.CL, ry, name_w, row_h * 0.48,
                     font, 11, 'FFFFFF', bold=True, align='center')
            _h2_text(slide, tier.get('price', ''),
                     _LY.CL, ry + row_h * 0.46, name_w, row_h * 0.52,
                     font, 26, 'FFFFFF', bold=True, align='center')
            tx = _LY.CL + name_w + 0.22
            tw = _LY.CW - name_w - 0.30
            feats = tier.get('features', [])[:5]
            feat_str = '   '.join(f'✓ {f}' for f in feats)
            _h2_text(slide, feat_str, tx, ry, tw, row_h,
                     font, 8, '555555', bold=False, align='left', line_spacing=1.4)

    elif v == 4:
        # v4 : cartes sombre en haut + corps clair — en-tête coloré + fonctionnalités bas
        gap    = _LY.GAP_SM
        col_w  = (_LY.CW - gap * (n - 1)) / n
        zone_h = _LY.CB - _LY.CT
        hdr_h  = zone_h * 0.42
        ...
        for i, tier in enumerate(tiers):
            ...
            color = accents[i % len(accents)]
            highlight = tier.get('highlight', False) or (n == 3 and i == 1)
            hdr_color = _darken(color, 0.78) if highlight else color
            _h2_rounded_rect(slide, cx, _LY.CT, col_w, zone_h, 'FFFFFF', _LY.R_SM)
            _h2_rounded_rect(slide, cx, _LY.CT, col_w, hdr_h + _LY.R_SM, hdr_color, _LY.R_SM)
            _h2_rect(slide, cx, _LY.CT + hdr_h * 0.6, col_w, hdr_h * 0.45, hdr_color)
            y = _LY.CT + 0.18
            _h2_text(slide, tier.get('name', f'Plan {i+1}').upper(),
                     cx + _LY.PAD, y, col_w - _LY.PAD * 2, 0.24,
                     font, 9, 'EEEEEE', bold=True, align='center')
            y += 0.26
            _h2_text(slide, tier.get('price', ''),
                     cx + _LY.PAD, y, col_w - _LY.PAD * 2, 0.50,
                     font, 34, 'FFFFFF', bold=True, align='center')
            y += 0.52
            _h2_text(slide, tier.get('period', 'par mois'),
                     cx + _LY.PAD, y, col_w - _LY.PAD * 2, 0.18,
                     font, 7, 'DDDDDD', bold=False, align='center')
            y = _LY.CT + hdr_h + 0.14
            for feat in tier.get('features', [])[:6]:
                tick_c = color
                _h2_text(slide, '✓', cx + _LY.PAD, y, 0.22, 0.24,
                         font, 10, tick_c, bold=True, align='center')
                _h2_text(slide, feat, cx + _LY.PAD + 0.20, y,
                         col_w - _LY.PAD - 0.26, 0.24,
                         font, 8, '444444', bold=False, align='left')
                y += 0.27
            if highlight:
                cta = tier.get('cta', 'Choisir')
                _h2_rounded_rect(slide, cx + _LY.PAD, _LY.CB - 0.40,
                                 col_w - _LY.PAD * 2, 0.28, color, 0.08)
                _h2_text(slide, cta, cx + _LY.PAD, _LY.CB - 0.40,
                         col_w - _LY.PAD * 2, 0.28, font, 9, 'FFFFFF', bold=True, align='center')
...
def layout_hub_spoke_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3 : hub carte gauche + items grille droite (colonnes)
        hub_w  = _LY.CW * 0.26
        hub_h  = _LY.CB - _LY.CT
        _h2_rounded_rect(slide, _LY.CL, _LY.CT, hub_w, hub_h, accents[0], _LY.R_MD)
        if cicon:
            _h2_text(slide, cicon, _LY.CL, _LY.CT + hub_h * 0.22,
                     hub_w, hub_h * 0.28, font, 36, 'FFFFFF', bold=False, align='center')
        _h2_text(slide, clabel, _LY.CL + 0.14, _LY.CT + hub_h * 0.52,
                 hub_w - 0.28, 0.44, font, 13, 'FFFFFF', bold=True, align='center')

        rx     = _LY.CL + hub_w + _LY.GAP_MD
        rw     = _LY.CR - rx
        n3     = min(n, 8)
        n_cols = 2
        n_rows = (n3 + 1) // 2
        ...
        for i, item in enumerate(items[:n3]):
            ...
            cy3 = iy + 0.10
            if icon:
                _h2_rounded_rect(slide, ix + 0.10, cy3, 0.32, 0.32, color, 0.16)
                _h2_text(slide, icon, ix + 0.10, cy3, 0.32, 0.32,
                         font, 12, 'FFFFFF', bold=False, align='center')
                _h2_text(slide, label, ix + 0.48, cy3 + 0.02, iw - 0.56, 0.26,
                         font, 9, dk1, bold=True, align='left')
            else:
                _h2_text(slide, label, ix + 0.12, cy3, iw - 0.20, 0.26,
                         font, 9, dk1, bold=True, align='left')
            if body:
                _h2_text(slide, body, ix + 0.12, cy3 + 0.30, iw - 0.18, ih - 0.46,
                         font, 8, '555555', bold=False, align='left')

    elif v == 4:
        # v4 : hub bandeau haut + accordion vertical avec connecteurs
        hub_h  = 0.78
        _h2_rounded_rect(slide, _LY.CL, _LY.CT, _LY.CW, hub_h, accents[0], _LY.R_SM)
        hub_cx = _LY.CL + _LY.CW / 2
        if cicon:
            _h2_text(slide, cicon, _LY.CL + 0.10, _LY.CT + (hub_h - 0.50) / 2,
                     0.50, 0.50, font, 22, 'FFFFFF', bold=False, align='center')
        lbl_x = _LY.CL + (0.66 if cicon else 0.20)
        _h2_text(slide, clabel, lbl_x, _LY.CT + (hub_h - 0.32) / 2,
                 _LY.CW - (lbl_x - _LY.CL) - 0.20, 0.32,
                 font, 14, 'FFFFFF', bold=True, align='left')

        n4      = min(n, 6)
        row_h   = (_LY.CB - _LY.CT - hub_h - _LY.GAP_SM * n4) / max(n4, 1)
        dot_x   = _LY.CL + 0.30
        _h2_rect(slide, dot_x - 0.01, _LY.CT + hub_h + _LY.GAP_SM,
                 0.02, row_h * n4 + _LY.GAP_SM * (n4 - 1), 'DDDDDD')
        for i, item in enumerate(items[:n4]):
            ...
            _h2_rect(slide, dot_x + 0.09, ry + row_h / 2 - 0.01, 0.47 - 0.09, 0.02, color)
            tx4 = _LY.CL + 0.56 + 0.12
            tw4 = _LY.CW - 0.56 - 0.20
            if icon:
                _h2_rounded_rect(slide, tx4, ry + (row_h - 0.28) / 2, 0.28, 0.28, color, 0.14)
                _h2_text(slide, icon, tx4, ry + (row_h - 0.28) / 2, 0.28, 0.28,
                         font, 11, 'FFFFFF', bold=False, align='center')
                tx4 += 0.36
                tw4 -= 0.36
            _h2_text(slide, label, tx4, ry + 0.06, tw4, 0.26,
                     font, 10, dk1, bold=True, align='left')
            if body:
                _h2_text(slide, body, tx4, ry + 0.34, tw4, row_h - 0.42,
...
def layout_competitor_matrix_v4(prs, content: dict, tp: dict):
    ...
    if v == 3:
        # v3 : carte par compétiteur — chaque compétiteur dans sa propre colonne
        n_cols = min(nc, 4)
        card_w = (_LY.CW - _LY.GAP_SM * (n_cols - 1)) / n_cols
        zone_h = _LY.CB - _LY.CT
        hdr_h  = 0.52
        feat_h = (zone_h - hdr_h - 0.10) / max(nf, 1)
        for ci, comp in enumerate(competitors[:n_cols]):
            cx    = _LY.CL + ci * (card_w + _LY.GAP_SM)
            color = accents[ci % len(accents)]
            _h2_card_bg(slide, cx, _LY.CT, card_w, zone_h, tp, ci)
            _h2_rounded_rect(slide, cx, _LY.CT, card_w, hdr_h, color, _LY.R_SM)
            _h2_rect(slide, cx, _LY.CT + hdr_h * 0.55, card_w, hdr_h * 0.50, color)
            _h2_text(slide, comp, cx + _LY.PAD, _LY.CT + 0.10,
                     card_w - _LY.PAD * 2, hdr_h - 0.14,
                     font, 10, 'FFFFFF', bold=True, align='center')
            y3 = _LY.CT + hdr_h + 0.08
            for fi, feat in enumerate(features[:nf]):
                ...
                val       = values[ci] if ci < len(values) else False
                if isinstance(val, str):
                    ...
                    sym, tc = '✗', 'CCCCCC'
                if fi % 2 == 0:
                    _h2_rect(slide, cx + 0.03, y3, card_w - 0.06, feat_h - 0.02, _cbg(tp, 0))
                _h2_text(slide, feat_name, cx + _LY.PAD, y3 + 0.04,
                         card_w - _LY.PAD * 2 - 0.22, feat_h - 0.06,
                         font, 7, dk1, bold=False, align='left')
                _h2_text(slide, sym, cx + card_w - 0.24, y3 + 0.04,
                         0.20, feat_h - 0.06, font, 10, tc, bold=True, align='center')
                y3 += feat_h
        return slide

    if v == 4:
        # v4 : heatmap — cellules colorées selon valeur ✓/✗
        hdr_h  = 0.46
        row_h  = min(max((_LY.CB - _LY.CT - hdr_h - _LY.GAP_SM) / max(nf, 1), 0.32), 0.54)
        name_w = _LY.CW * 0.30
        col_w4 = (_LY.CW - name_w) / max(nc, 1)
        for ci, comp in enumerate(competitors):
            cx    = _LY.CL + name_w + ci * col_w4
            color = accents[ci % len(accents)]
            _h2_rounded_rect(slide, cx + 0.03, _LY.CT, col_w4 - 0.06, hdr_h, color, _LY.R_SM)
            _h2_text(slide, comp, cx, _LY.CT + 0.08, col_w4, hdr_h - 0.10,
                     font, 9, 'FFFFFF', bold=True, align='center')
        for fi, feat in enumerate(features):
            ...
            for ci in range(nc):
                ...
                color = accents[ci % len(accents)]
                if isinstance(val, str):
                    sym, cell_bg = val, _cbg(tp, 0)
                    tc = color
                elif val:
                    r4, g4, b4 = int(color[:2], 16), int(color[2:4], 16), int(color[4:], 16)
                    cell_bg = f'{min(255,int(r4+(255-r4)*0.82)):02X}{min(255,int(g4+(255-g4)*0.82)):02X}{min(255,int(b4+(255-b4)*0.82)):02X}'
                    sym, tc = '✓', color
                else:
                    cell_bg, sym, tc = 'F5F5F5', '✗', 'CCCCCC'
                _h2_rect(slide, cx + 0.02, fy, col_w4 - 0.04, row_h - 0.02, cell_bg)
                _h2_text(slide, sym, cx, fy + 0.04, col_w4, row_h - 0.08,
                         font, 12, tc, bold=True, align='center')
        return slide
...
def layout_pest_analysis_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3 : accordion rows — panneau gauche lettre colorée + body droite
        zone_h = _LY.CB - _LY.CT
        row_h  = (zone_h - _LY.GAP_XS * (n - 1)) / n
        ltr_w  = 1.30
        for i, item in enumerate(items):
            ...
            body   = item.get('body', '')            if isinstance(item, dict) else ''
            _h2_rounded_rect(slide, _LY.CL, ry, _LY.CW, row_h, 'F4F7FA', _LY.R_SM)
            _h2_rounded_rect(slide, _LY.CL, ry, ltr_w, row_h, color, _LY.R_SM)
            _h2_rect(slide, _LY.CL + ltr_w - 0.08, ry, 0.08, row_h, color)
            _h2_text(slide, letter, _LY.CL, ry, ltr_w, row_h,
                     font, 52, 'FFFFFF', bold=True, align='center')
            tx3 = _LY.CL + ltr_w + 0.20
            tw3 = _LY.CW - ltr_w - 0.28
            _h2_text(slide, label, tx3, ry + (row_h - 0.30) * 0.24, tw3, 0.30,
                     font, _LY.T_HEADER, color, bold=True, align='left')
            if body:
                _h2_text(slide, body, tx3, ry + 0.40, tw3, row_h - 0.48,
                         font, _LY.T_SMALL, dk1, bold=False, align='left', line_spacing=1.2)

    elif v == 4:
        # v4 : colonnes fond coloré plein + lettre fantôme blanc + label + body
        gap    = _LY.GAP_SM
        tile_w = (_LY.CW - gap * (n - 1)) / n
        tile_h = _LY.CB - _LY.CT
        for i, item in enumerate(items):
            ...
            icon   = item.get('icon', '')            if isinstance(item, dict) else ''
            body   = item.get('body', '')            if isinstance(item, dict) else ''
            _h2_rounded_rect(slide, cx, _LY.CT, tile_w, tile_h, color, _LY.R_SM)
            # Ghost letter watermark (white, low contrast)
            r4, g4, b4 = int(color[:2], 16), int(color[2:4], 16), int(color[4:], 16)
            ghost4 = (f'{min(255,int(r4+(255-r4)*0.30)):02X}'
                      f'{min(255,int(g4+(255-g4)*0.30)):02X}'
                      f'{min(255,int(b4+(255-b4)*0.30)):02X}')
            _h2_text(slide, letter, cx - 0.04, _LY.CT - 0.14,
                     tile_w + 0.08, tile_h * 0.60,
                     font, 90, ghost4, bold=True, align='left')
            y4 = _LY.CT + tile_h * 0.48
            if icon:
                _h2_rounded_rect(slide, cx + tile_w / 2 - 0.20, y4, 0.40, 0.40,
                                 _darken(color, 0.75), 0.20)
                _h2_text(slide, icon, cx + tile_w / 2 - 0.20, y4, 0.40, 0.40,
                         font, 16, 'FFFFFF', bold=False, align='center')
                y4 += 0.46
            _h2_text(slide, label, cx + _LY.PAD, y4, tile_w - _LY.PAD * 2, 0.28,
                     font, 10, 'FFFFFF', bold=True, align='center')
            y4 += 0.30
            if body:
                _h2_text(slide, body, cx + _LY.PAD, y4, tile_w - _LY.PAD * 2,
                         _LY.CB - y4 - 0.10, font, 8, 'EEEEEE', bold=False, align='center')
...
def layout_diamond_icons_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3 : accordion rows — losange gauche + titre/body droite
        zone_h = _LY.CB - _LY.CT
        row_h  = (zone_h - _LY.GAP_XS * (n - 1)) / n
        d_size = min(row_h * 0.68, 0.90)
        for i, item in enumerate(items):
            ry    = _LY.CT + i * (row_h + _LY.GAP_XS)
            color = accents[i % len(accents)]
            icon  = item.get('icon',  '') if isinstance(item, dict) else ''
            ...
            body  = item.get('body',  '') if isinstance(item, dict) else ''
            _h2_rounded_rect(slide, _LY.CL, ry, _LY.CW, row_h, 'F4F7FA', _LY.R_SM)
            dx = _LY.CL + (1.30 - d_size) / 2
            dy = ry + (row_h - d_size) / 2
            _h2_diamond(slide, dx, dy, d_size, d_size, color)
            if icon:
                _h2_text(slide, icon, dx, dy + d_size * 0.20, d_size, d_size * 0.58,
                         font, int(d_size * 22), 'FFFFFF', bold=False, align='center')
            tx3 = _LY.CL + 1.36
            tw3 = _LY.CW - 1.42
            _h2_text(slide, ttxt, tx3, ry + (row_h - 0.30) * 0.24, tw3, 0.30,
                     font, _LY.T_HEADER, dk1, bold=True, align='left')
            if body:
                _h2_text(slide, body, tx3, ry + 0.40, tw3, row_h - 0.48,
                         font, _LY.T_SMALL, '555555', bold=False, align='left',
        ...
        # v4 : cartes fond coloré + losange en haut + titre + body
        gap    = _LY.GAP_SM
        card_w = (_LY.CW - gap * (n - 1)) / n
        zone_h = _LY.CB - _LY.CT
        d_size = min(card_w * 0.46, 0.96)
        for i, item in enumerate(items):
            cx    = _LY.CL + i * (card_w + gap)
            color = accents[i % len(accents)]
            icon  = item.get('icon',  '') if isinstance(item, dict) else ''
            ttxt  = item.get('title', str(item)) if isinstance(item, dict) else str(item)
            body  = item.get('body',  '') if isinstance(item, dict) else ''
            _h2_rounded_rect(slide, cx, _LY.CT, card_w, zone_h, color, _LY.R_SM)
            dx4 = cx + (card_w - d_size) / 2
            dy4 = _LY.CT + 0.18
            r4, g4, b4 = int(color[:2], 16), int(color[2:4], 16), int(color[4:], 16)
            dark_c = f'{max(0,int(r4*0.72)):02X}{max(0,int(g4*0.72)):02X}{max(0,int(b4*0.72)):02X}'
            _h2_diamond(slide, dx4, dy4, d_size, d_size, dark_c)
            if icon:
                _h2_text(slide, icon, dx4, dy4 + d_size * 0.20, d_size, d_size * 0.58,
                         font, int(d_size * 22), 'FFFFFF', bold=False, align='center')
            ty4 = dy4 + d_size + 0.16
            _h2_rect(slide, cx + _LY.PAD, ty4, card_w - _LY.PAD * 2, 0.03, 'FFFFFF')
            ty4 += 0.10
            _h2_text(slide, ttxt, cx + _LY.PAD, ty4, card_w - _LY.PAD * 2, 0.28,
                     font, 10, 'FFFFFF', bold=True, align='center')
            ty4 += 0.30
            if body:
                _h2_text(slide, body, cx + _LY.PAD, ty4, card_w - _LY.PAD * 2,
                         zone_h - (ty4 - _LY.CT) - 0.10,
...
def layout_market_sizing_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        zone_h1 = _LY.CB - _LY.CT
        ch_full = (zone_h1 - gap * (n_rows - 1)) / max(n_rows, 1)
        ch_est  = 1.28
        ch      = max(ch_est, ch_full * 0.36)
        _y_off  = (ch_full - ch) * 0.28 if n_rows == 1 and ch < ch_full * 0.65 else 0.0

        for i, seg in enumerate(segments):
            col_i  = i % n_cols
            row_i  = i // n_cols
            cx     = _LY.CL + col_i * (cw + gap)
            cy     = _LY.CT + _y_off + row_i * (ch + gap)
        ...
        # v2: Nested concentric rectangles (TAM outer → SOM inner) + labels left (TAM outer → SOM inner) + labels left
        lbl_w  = _LY.CW * 0.34
        ...
        # v3 : barres horizontales — label gauche + barre + valeur
        zone_h = _LY.CB - _LY.CT
        row_h  = (zone_h - _LY.GAP_XS * (n - 1)) / n
        lbl_w  = 1.60
        for i, seg in enumerate(segments):
            ry     = _LY.CT + i * (row_h + _LY.GAP_XS)
            label  = seg.get('label',  f'Seg {i+1}') if isinstance(seg, dict) else str(seg)
            value  = seg.get('value',  '')            if isinstance(seg, dict) else ''
            change = seg.get('change', '')            if isinstance(seg, dict) else ''
            trend  = seg.get('trend',  'up')          if isinstance(seg, dict) else 'up'
            desc   = seg.get('description', '')       if isinstance(seg, dict) else ''
            ...
            arrow  = '▲' if up else '▼'
            ac     = '27AE60' if up else 'E74C3C'
            _h2_rounded_rect(slide, _LY.CL + lbl_w, ry + _LY.GAP_XS,
                             _LY.CW - lbl_w, row_h - _LY.GAP_XS * 2, 'EEEEEE', _LY.R_SM)
            bar_w  = (_LY.CW - lbl_w) * (1.0 - i * 0.12)
            _h2_rounded_rect(slide, _LY.CL + lbl_w, ry + _LY.GAP_XS,
                             bar_w, row_h - _LY.GAP_XS * 2, color, _LY.R_SM)
            _h2_text(slide, label, _LY.CL, ry + (row_h - 0.30) * 0.20,
                     lbl_w - 0.10, 0.30, font, 10, dk1, bold=True, align='right')
            _h2_text(slide, value, _LY.CL + lbl_w + bar_w - 1.60,
                     ry + (row_h - 0.36) / 2, 1.50, 0.36,
                     font, 18, 'FFFFFF', bold=True, align='right')
            if change:
                _h2_text(slide, f'{arrow} {change}',
                         _LY.CL + lbl_w + _LY.GAP_SM, ry + (row_h - 0.22) * 0.75,
                         1.20, 0.22, font, 8, 'FFFFFF', bold=True, align='left')

    elif v == 4:
        # v4 : cartes KPI compactes fond coloré + valeur géante + label + trend
        gap    = _LY.GAP_SM
        n_cols = min(n, 4)
        card_w = (_LY.CW - gap * (n_cols - 1)) / n_cols
        zone_h = _LY.CB - _LY.CT
        for i, seg in enumerate(segments[:n_cols]):
            cx     = _LY.CL + i * (card_w + gap)
            label  = seg.get('label',  f'Seg {i+1}') if isinstance(seg, dict) else str(seg)
            value  = seg.get('value',  '')            if isinstance(seg, dict) else ''
            change = seg.get('change', '')            if isinstance(seg, dict) else ''
            ...
            up     = trend != 'down'
            arrow  = '▲' if up else '▼'
            _h2_rounded_rect(slide, cx, _LY.CT, card_w, zone_h, color, _LY.R_MD)
            _h2_text(slide, label, cx + _LY.PAD, _LY.CT + 0.18,
                     card_w - _LY.PAD * 2, 0.28,
                     font, 10, 'EEEEEE', bold=True, align='center')
            _h2_text(slide, value, cx + _LY.PAD, _LY.CT + zone_h * 0.28,
                     card_w - _LY.PAD * 2, zone_h * 0.38,
                     font, _LY.T_KPI, 'FFFFFF', bold=True, align='center')
            _h2_rect(slide, cx + card_w * 0.25, _LY.CT + zone_h * 0.68,
                     card_w * 0.50, 0.04, 'FFFFFF')
            if change:
                ac4 = 'AAFFAA' if up else 'FFAAAA'
                _h2_text(slide, f'{arrow} {change}', cx + _LY.PAD,
                         _LY.CT + zone_h * 0.72, card_w - _LY.PAD * 2, 0.28,
                         font, 10, ac4, bold=True, align='center')
            if desc:
                _h2_text(slide, desc, cx + _LY.PAD, _LY.CT + zone_h * 0.80,
                         card_w - _LY.PAD * 2, zone_h * 0.16,
                         font, 8, 'DDDDDD', bold=False, align='center')
...
def layout_chevron_flow_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3 : accordion rows — chevron tronqué gauche + titre/body droite
        zone_h  = _LY.CB - _LY.CT
        row_h   = (zone_h - _LY.GAP_XS * (n - 1)) / n
        chev_w  = 2.20
        for i, step in enumerate(steps):
            ...
            sv    = step.get('stat_value', '') if isinstance(step, dict) else ''
            _h2_rounded_rect(slide, _LY.CL, ry, _LY.CW, row_h, 'F4F7FA', _LY.R_SM)
            _h2_chevron(slide, _LY.CL, ry, chev_w, row_h, color)
            if icon:
                _h2_text(slide, icon, _LY.CL, ry, chev_w * 0.56, row_h,
                         font, int(row_h * 22), 'FFFFFF', bold=False, align='center')
            else:
                _h2_text(slide, str(i + 1), _LY.CL, ry, chev_w * 0.56, row_h,
                         font, int(row_h * 28), 'FFFFFF', bold=True, align='center')
            tx3 = _LY.CL + chev_w + 0.16
            tw3 = _LY.CW - chev_w - 0.22 - (1.20 if sv else 0)
            _h2_text(slide, ttxt, tx3, ry + (row_h - 0.30) * 0.22, tw3, 0.30,
                     font, _LY.T_HEADER, dk1, bold=True, align='left')
            if body:
                _h2_text(slide, body, tx3, ry + 0.38, tw3, row_h - 0.46,
                         font, _LY.T_SMALL, '555555', bold=False, align='left',
                         line_spacing=1.2)
            if sv:
                _h2_text(slide, sv, _LY.CR - 1.10, ry + (row_h - 0.38) / 2,
                         1.00, 0.38, font, 20, color, bold=True, align='right')

    elif v == 4:
        # v4 : timeline numérotée — ligne verticale + cartes étapes
        dot_x   = _LY.CL + 0.40
        zone_h  = _LY.CB - _LY.CT
        row_h   = (zone_h - _LY.GAP_SM * (n - 1)) / n
        _h2_rect(slide, dot_x - 0.01, _LY.CT, 0.02, zone_h, 'DDDDDD')
        for i, step in enumerate(steps):
            ry    = _LY.CT + i * (row_h + _LY.GAP_SM)
            color = accents[i % len(accents)]
            icon  = step.get('icon', '')  if isinstance(step, dict) else ''
            ttxt  = step.get('title', str(step)) if isinstance(step, dict) else str(step)
            body  = step.get('body', '')  if isinstance(step, dict) else ''
            sv    = step.get('stat_value', '') if isinstance(step, dict) else ''
            sl    = step.get('stat_label', '') if isinstance(step, dict) else ''
            _h2_rounded_rect(slide, _LY.CL, ry + row_h / 2 - 0.22,
                             0.80, 0.44, color, _LY.R_SM)
            _h2_text(slide, str(i + 1), _LY.CL, ry + row_h / 2 - 0.22,
                     0.80, 0.44, font, 16, 'FFFFFF', bold=True, align='center')
            _h2_rect(slide, dot_x + 0.01, ry + row_h / 2 - 0.01,
                     0.38, 0.02, color)
            cx4 = _LY.CL + 0.86
            cw4 = _LY.CW - 0.86 - (1.10 if sv else 0)
            _h2_rounded_rect(slide, cx4, ry + _LY.GAP_XS,
                             _LY.CW - 0.86, row_h - _LY.GAP_XS * 2, 'F8F8F8', _LY.R_SM)
            _h2_rect(slide, cx4, ry + _LY.GAP_XS, 0.04, row_h - _LY.GAP_XS * 2, color)
            if icon:
                _h2_text(slide, icon, cx4 + 0.10, ry + (row_h - 0.36) / 2,
                         0.36, 0.36, font, 16, color, bold=False, align='center')
                tx4 = cx4 + 0.52
            else:
                tx4 = cx4 + 0.14
            _h2_text(slide, ttxt, tx4, ry + (row_h - 0.28) * 0.22, cw4 - 0.10, 0.28,
                     font, _LY.T_HEADER, dk1, bold=True, align='left')
            if body:
                _h2_text(slide, body, tx4, ry + 0.36, cw4 - 0.10, row_h - 0.44,
                         font, _LY.T_SMALL, '666666', bold=False, align='left',
                         line_spacing=1.2)
            if sv:
                _h2_text(slide, sv, _LY.CR - 1.00, ry + (row_h - 0.36) / 2,
                         0.90, 0.36, font, 18, color, bold=True, align='right')
                if sl:
                    _h2_text(slide, sl, _LY.CR - 1.00, ry + (row_h - 0.36) / 2 + 0.28,
                             0.90, 0.18, font, 7, '999999', bold=False, align='right')
...
def layout_venn_v4(prs, content: dict, tp: dict):
    ...
    if v == 0 or (n < 3 and v == 1):
        ...
        # v3 : deux panneaux colorés avec zone intersection blanche au centre
        c1      = circles[0] if n >= 1 else {}
        c2      = circles[1] if n >= 2 else {}
        zone_h  = _LY.CB - _LY.CT
        ov_w    = 1.30
        pw3     = (_LY.CW - ov_w) / 2
        ...
        color2  = (c2.get('color', accents[1 % len(accents)])
                   if isinstance(c2, dict) else accents[1 % len(accents)])
        _h2_rounded_rect(slide, _LY.CL, _LY.CT, pw3 + ov_w / 2, zone_h, color1, _LY.R_MD)
        _h2_rounded_rect(slide, _LY.CL + pw3 + ov_w / 2, _LY.CT,
                         pw3 + ov_w / 2, zone_h, color2, _LY.R_MD)
        cx3 = _LY.CL + pw3
        _h2_rounded_rect(slide, cx3, _LY.CT + zone_h * 0.08,
                         ov_w, zone_h * 0.84, 'FFFFFF', _LY.R_SM)
        inter_lbl  = inter.get('label', '') if isinstance(inter, dict) else ''
        inter_icon = inter.get('icon', '')  if isinstance(inter, dict) else ''
        iy3 = _LY.CT + zone_h * 0.38
        if inter_icon:
            _h2_text(slide, inter_icon, cx3, iy3, ov_w, 0.38, font, 18, dk1,
                     bold=False, align='center')
            iy3 += 0.40
        if inter_lbl:
            _h2_text(slide, inter_lbl, cx3, iy3, ov_w, 0.28, font, 9, dk1,
                     bold=True, align='center')
        for ci, (circ, color3, lx3, lw3, al3) in enumerate([
                (c1, color1, _LY.CL + 0.18, pw3 - 0.18, 'left'),
                (c2, color2, cx3 + ov_w + 0.06, pw3 - 0.12, 'left'),
        ]):
            lbl3  = circ.get('label', '') if isinstance(circ, dict) else str(circ)
            icon3 = circ.get('icon', '')  if isinstance(circ, dict) else ''
            its3  = circ.get('items', []) if isinstance(circ, dict) else []
            y3    = _LY.CT + 0.20
            if icon3:
                _h2_text(slide, icon3, lx3, y3, lw3, 0.40, font, 20, 'FFFFFF',
                         bold=False, align=al3)
                y3 += 0.44
            _h2_text(slide, lbl3, lx3, y3, lw3, 0.30, font, 12, 'FFFFFF',
                     bold=True, align=al3)
            y3 += 0.34
            for it in its3[:5]:
                _h2_text(slide, f'• {it}', lx3, y3, lw3, 0.24, font, 8, 'FFFFFF',
                         bold=False, align=al3)
                y3 += 0.26

    elif v == 4:
        # v4 : cartes côte-à-côte — chaque cercle comme panneau coloré avec icône
        nc4    = min(n, 3)
        pw4    = (_LY.CW - _LY.GAP_SM * (nc4 - 1)) / nc4
        zone_h = _LY.CB - _LY.CT
        for i in range(nc4):
            c4     = circles[i]
            cx4    = _LY.CL + i * (pw4 + _LY.GAP_SM)
            color4 = (c4.get('color', accents[i % len(accents)])
                      if isinstance(c4, dict) else accents[i % len(accents)])
            lbl4   = c4.get('label', '') if isinstance(c4, dict) else str(c4)
            icon4  = c4.get('icon', '')  if isinstance(c4, dict) else ''
            its4   = c4.get('items', []) if isinstance(c4, dict) else []
            _h2_rounded_rect(slide, cx4, _LY.CT, pw4, zone_h, color4, _LY.R_MD)
            r4 = min(pw4 * 0.38, zone_h * 0.34)
            ri4, gi4, bi4 = int(color4[:2],16), int(color4[2:4],16), int(color4[4:],16)
            dk4 = (f'{max(0,int(ri4*0.72)):02X}'
                   f'{max(0,int(gi4*0.72)):02X}'
                   f'{max(0,int(bi4*0.72)):02X}')
            _h2_rounded_rect(slide, cx4 + pw4 / 2 - r4, _LY.CT + zone_h * 0.06,
                             r4 * 2, r4 * 2, dk4, r4)
            if icon4:
                _h2_text(slide, icon4, cx4 + pw4 / 2 - r4,
                         _LY.CT + zone_h * 0.06, r4 * 2, r4 * 2,
                         font, int(r4 * 28), 'FFFFFF', bold=False, align='center')
            y4 = _LY.CT + zone_h * 0.06 + r4 * 2 + 0.14
            _h2_text(slide, lbl4, cx4 + _LY.PAD, y4, pw4 - _LY.PAD * 2, 0.30,
                     font, 12, 'FFFFFF', bold=True, align='center')
            y4 += 0.34
            for it in its4[:5]:
                _h2_text(slide, f'• {it}', cx4 + _LY.PAD, y4, pw4 - _LY.PAD * 2,
                         0.24, font, 8, 'EEEEEE', bold=False, align='center')
                y4 += 0.26
...
def layout_icon_grid_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: feature list cards — left color strip + icon circle + label + body
        n_cols = 3 if n >= 7 else 2
        n_rows = (n + n_cols - 1) // n_cols
        gap_x  = _LY.GAP_SM
        gap_y  = _LY.GAP_XS
        cell_w = (_LY.CW - gap_x * (n_cols - 1)) / n_cols
        cell_h = (_LY.CB - _LY.CT - gap_y * (n_rows - 1)) / max(n_rows, 1)
        r_icon = min(cell_h * 0.28, 0.28)
        ...
        for i, item in enumerate(items):
            ...
            _h2_rounded_rect(slide, cx, cy, 0.10, cell_h, color, _LY.R_SM)
            _h2_rect(slide, cx + _LY.R_SM, cy, 0.10 - _LY.R_SM, cell_h, color)

            icy = cy + (cell_h - r_icon * 2) / 2
            _h2_rounded_rect(slide, cx + 0.20, icy, r_icon * 2, r_icon * 2, color, r_icon)
            if icon:
                _h2_text(slide, icon, cx + 0.20, icy, r_icon * 2, r_icon * 2,
                         font, int(r_icon * 24), 'FFFFFF', bold=False, align='center')

            tx = cx + 0.20 + r_icon * 2 + 0.12
            tw = cell_w - (tx - cx) - 0.08
            ty = cy + (cell_h - (0.48 if body else 0.26)) / 2
            _h2_text(slide, label, tx, ty, tw, 0.26,
                     font, 10, dk1, bold=True, align='left')
            if body:
                _h2_text(slide, body, tx, ty + 0.28, tw,
                         cell_h - (ty - cy) - 0.34, font, 8, '555555', bold=False, align='left')

    elif v == 4:
        # v4: full-color tiles — colored rounded rect + icon + label below
        n_cols = 4 if n >= 9 else (3 if n >= 5 else 2)
        n_rows = (n + n_cols - 1) // n_cols
        gap_x  = _LY.GAP_SM
        gap_y  = _LY.GAP_SM
        cell_w = (_LY.CW - gap_x * (n_cols - 1)) / n_cols
        ...
        for i, item in enumerate(items):
            ...
            r_ic = min(cell_w * 0.24, cell_h * 0.28, 0.36)
            icx  = cx + cell_w / 2 - r_ic
            icy  = cy + cell_h * 0.12
            if icon:
                _h2_text(slide, icon, icx, icy, r_ic * 2, r_ic * 2,
                         font, int(r_ic * 28), 'FFFFFF', bold=False, align='center')
            ty = cy + cell_h * 0.56
            _h2_text(slide, label, cx + 0.06, ty, cell_w - 0.12, 0.26,
                     font, 9, 'FFFFFF', bold=True, align='center')
            if body and cell_h > 1.10:
                _h2_text(slide, body, cx + 0.06, ty + 0.28, cell_w - 0.12,
                         cell_h - (ty - cy) - 0.34, font, 7, 'EEEEEE', bold=False, align='center')
...
def layout_text_hero_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: full dark background — white hero word + accent underline
        W3 = tp.get('W', 13.33)
        _h2_rect(slide, 0, 0, W3, 7.50, dk1)
        bg_r = zone_h * 0.60
        _h2_rounded_rect(slide, _LY.CR - bg_r * 0.85, _LY.CT + zone_h / 2 - bg_r,
                         bg_r * 2, bg_r * 2, color, bg_r)
        _set_shape_alpha(slide.shapes[-1], 14)
        if title:
            _h2_text(slide, title.upper(), _LY.CL, _LY.CT + 0.08, _LY.CW, 0.26,
                     font, 9, color, bold=True, align='left')
        _h2_text(slide, hero, _LY.CL - 0.05, _LY.CT + 0.24, _LY.CW * 0.80, zone_h * 0.60,
                 font, 90, 'FFFFFF', bold=True, align='left')
        ay = _LY.CT + zone_h * 0.62
        _h2_rect(slide, _LY.CL, ay, _LY.CW * 0.42, 0.055, color)
        y = ay + 0.12
        if subtitle:
            _h2_text(slide, subtitle, _LY.CL, y, _LY.CW * 0.70, 0.28,
                     font, 11, color, bold=True, align='left')
            y += 0.30
        if body:
            _h2_text(slide, body, _LY.CL, y, _LY.CW * 0.70, _LY.CB - y - 0.05,
                     font, 10, 'AAAAAA', bold=False, align='left')

    elif v == 4:
        # v4: magazine split — accent left panel + hero + body right
        W4 = tp.get('W', 13.33)
        split_x = _LY.CL + _LY.CW * 0.38
        _h2_rect(slide, 0, 0, split_x, 7.50, color)
        lw = split_x - _LY.CL - 0.15
        if title:
            _h2_text(slide, title.upper(), _LY.CL + 0.25, _LY.CT + 0.10, lw, 0.26,
                     font, 9, 'FFFFFF', bold=True, align='left')
        _h2_text(slide, hero, _LY.CL + 0.20, _LY.CT + 0.30, lw, zone_h * 0.56,
                 font, 72, 'FFFFFF', bold=True, align='left')
        ay4 = _LY.CT + zone_h * 0.60
        _h2_rect(slide, _LY.CL + 0.25, ay4, lw * 0.55, 0.05, 'FFFFFF')
        _set_shape_alpha(slide.shapes[-1], 60)
        if subtitle:
            _h2_text(slide, subtitle, _LY.CL + 0.25, ay4 + 0.12, lw, 0.28,
                     font, 10, 'FFFFFF', bold=True, align='left')
        rx = split_x + 0.35
        rw = _LY.CR - rx
        ry = _LY.CT + zone_h * 0.15
        if accent_w:
            _h2_text(slide, accent_w, rx, ry, rw, zone_h * 0.38,
                     font, 60, color, bold=True, align='left')
            ry += zone_h * 0.40
        if body:
            _h2_text(slide, body, rx, ry, rw, _LY.CB - ry - 0.05,
                     font, 11, dk1, bold=False, align='left')
        elif subtitle and not accent_w:
            _h2_text(slide, subtitle, rx, ry, rw, _LY.CB - ry - 0.05,
                     font, 13, dk1, bold=True, align='left')
...
def layout_org_chart_v4(prs, content: dict, tp: dict):
    ...
    if v == 0:
        ...
        # v3: card-style — L1 children as two-tone cards (colored header + white body with grandchildren)
        ...
        l1_total = CARD_W * nc + _LY.GAP_SM * (nc - 1)
        l1_y     = root_cy + BOX_H / 2 + 0.70
        l1_xs    = [W / 2 - l1_total / 2 + i * (CARD_W + _LY.GAP_SM) + CARD_W / 2 for i in range(nc)]
        bus_y3   = root_cy + BOX_H / 2 + 0.32

        _vline(root_cx, root_cy + BOX_H / 2, bus_y3)
        if nc > 1:
            _hline(l1_xs[0], l1_xs[-1], bus_y3)
        ...
        for i, child in enumerate(children):
            ...
            bx = cx_ - CARD_W / 2
            by = l1_y - CARD_H / 2
            _h2_rounded_rect(slide, bx, by, CARD_W, CARD_H, color_i, _LY.R_SM)
            _h2_rounded_rect(slide, bx, by + HDRH, CARD_W, CARD_H - HDRH, 'FFFFFF', _LY.R_SM)
            _h2_rect(slide, bx, by + HDRH, CARD_W, _LY.R_SM, 'FFFFFF')
            if c_icon:
                _h2_text(slide, c_icon, bx + 0.06, by + 0.02, 0.30, HDRH - 0.04,
                         font, 12, 'FFFFFF', bold=False, align='center')
            _h2_text(slide, c_label, bx + (0.36 if c_icon else 0.08), by + 0.04,
                     CARD_W - (0.44 if c_icon else 0.16), HDRH - 0.08,
                     font, 9, 'FFFFFF', bold=True, align='left')
            gcs = child.get('children', []) if isinstance(child, dict) else []
            ng  = min(len(gcs), 3)
            if ng > 0:
        ...
        # v4: flat team grid — all L1 children as equal colored tiles with avatar circle + label
        n_show = max(nc, 1)
        TILE_W = min((_LY.CW - _LY.GAP_SM * (n_show - 1)) / n_show, 2.40)
        TILE_H = min(_LY.CB - _LY.CT - 1.20, 2.20)
        R_TILE = min(TILE_W * 0.28, TILE_H * 0.30, 0.50)
        tile_y = _LY.CT + (_LY.CB - _LY.CT - TILE_H) / 2 + 0.20

        _h2_text(slide, root_label, _LY.CL, _LY.CT, _LY.CW, 0.40,
                 font, _LY.T_LABEL, root_color, bold=True, align='center')
        _h2_rect(slide, _LY.CL + _LY.CW * 0.35, _LY.CT + 0.42, _LY.CW * 0.30, 0.04, root_color)

        tile_total = TILE_W * n_show + _LY.GAP_SM * (n_show - 1)
        x0 = W / 2 - tile_total / 2
        ...
        for i, child in enumerate(children):
            ...
            icy = tile_y + TILE_H * 0.12
            _h2_rounded_rect(slide, icx, icy, R_TILE * 2, R_TILE * 2, 'FFFFFF', R_TILE)
            if c_icon:
                r_i = int(color_i[0:2], 16)
                g_i = int(color_i[2:4], 16)
                b_i = int(color_i[4:6], 16)
                dark_c = (f'{max(0, int(r_i * 0.70)):02X}'
                          f'{max(0, int(g_i * 0.70)):02X}'
                          f'{max(0, int(b_i * 0.70)):02X}')
                _h2_text(slide, c_icon, icx, icy, R_TILE * 2, R_TILE * 2,
                         font, int(R_TILE * 24), dark_c, bold=False, align='center')
            _h2_text(slide, c_label, tx + 0.08, icy + R_TILE * 2 + 0.10, TILE_W - 0.16, 0.28,
                     font, 10, 'FFFFFF', bold=True, align='center')
            gcs = child.get('children', []) if isinstance(child, dict) else []
            ng  = min(len(gcs), 3)
...
bar_chart      — Barres groupées  → categories:[str] + series:[{{name,values:[n]}}] + analysis(2 phrases)
line_chart     — Courbes          → categories:[str] + series:[{{name,values:[n]}}] + analysis(2 phrases)
pie_chart      — Camembert/anneau → slices:[{{label,value}}] + analysis(2 phrases) + doughnut?:bool
stacked_bar    — Barres empilées  → categories:[str] + series:[{{name,values:[n]}}] + analysis(2 phrases)
waterfall      — Cascade          → items:[{{label,value}}] (positifs=gains, négatifs=pertes) + analysis
radar          — Radar            → axes:[str] + series:[{{name,values:[n]}}] + analysis(2 phrases)
  Optionnel pour tous : key_metrics:[{{label,value}}]×1-3 (affichés dans la sidebar en variante v1)
